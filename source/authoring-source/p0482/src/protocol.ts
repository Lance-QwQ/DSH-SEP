...
export interface MemoryControls { readonly capture: boolean; readonly recall: boolean }
/** Configured project identity returned by the Host. */
export interface MemoryProject { readonly id: string; readonly label: string; readonly root: string }
/** List response for this protocol version. */
export interface MemoryProjectList { readonly version: 1; readonly available: boolean; readonly reason: string | null; readonly projects: readonly MemoryProject[] }
...
export interface MemorySnapshot {
  ...
  readonly runtime: { readonly state: 'ready' | 'degraded' | 'disabled' | 'unavailable'; readonly lastError: string | null };
