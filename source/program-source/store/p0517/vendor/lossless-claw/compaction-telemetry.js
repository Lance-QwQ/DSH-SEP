...
export class CompactionTelemetryRecorder {
    ...
    deps;
    constructor(compactionTelemetryStore, compactionMaintenanceStore, deps) {
        this.compactionTelemetryStore = compactionTelemetryStore;
        this.compactionMaintenanceStore = compactionMaintenanceStore;
        this.deps = deps;
    }
    /** Extract the current prompt-cache snapshot from runtime context, if present. */
    readPromptCacheSnapshot(runtimeContext) {
        ...
        const lastCacheTouchAt = typeof lastCacheTouchAtRaw === "number" && Number.isFinite(lastCacheTouchAtRaw)
            ? new Date(lastCacheTouchAtRaw)
            : undefined;
        const hasUsageSignal = cacheRead !== undefined || cacheWrite !== undefined;
        const hasObservationSignal = typeof observation?.cacheRead === "number"
    ...
    async updateCompactionTelemetry(params) {
        ...
        const existing = await this.compactionTelemetryStore.getConversationCompactionTelemetry(params.conversationId);
        if (!snapshot && params.rawTokensOutsideTail === undefined) {
            return existing;
        }
        const now = new Date();
        const turnsSinceLeafCompaction = (existing?.turnsSinceLeafCompaction ?? 0) + 1;
        const tokensAccumulatedSinceLeafCompaction = params.rawTokensOutsideTail ?? existing?.tokensAccumulatedSinceLeafCompaction ?? 0;
        const touchedPromptCache = snapshot?.lastCacheTouchAt
            ?? (snapshot
        ...
        await this.compactionTelemetryStore.upsertConversationCompactionTelemetry({
            ...
            lastObservedCacheWrite: snapshot?.lastObservedCacheWrite ?? existing?.lastObservedCacheWrite ?? null,
            lastObservedPromptTokenCount: snapshot?.lastObservedPromptTokenCount ?? existing?.lastObservedPromptTokenCount ?? null,
            lastObservedCacheHitAt: snapshot?.cacheState === "hot"
                ? now
    ...
    async markLeafCompactionTelemetrySuccess(params) {
        ...
        this.deps.log.debug(`[lcm] compaction telemetry reset after compaction: conversation=${params.conversationId} cacheState=${existing?.cacheState ?? "unknown"} activityBand=${existing?.lastActivityBand ?? "low"}`);
    }
