...
import { getConfigValue, readRawLosslessConfig, readConfigView, setConfigValue, } from "./config-file.js";
import { openReadOnlyDatabase } from "./database.js";
import { CliError, createErrorEnvelope, createSuccessEnvelope, normalizeCliError, renderSuccessEnvelope, } from "./output.js";
import { resolveCliPaths } from "./paths.js";
import { getConversationDiagnostics, getFreshTail, getGlobalStatus, getSummaryDetails, listConversations, listMessages, listSummaries, } from "./queries.js";
...
function loadConfigView(configPath, env) {
...
function resolveInvocationContext(parsed, env) {
...
function runConfigSetCommand(parsed, command, env) {
    const overrides = pathOverrides(parsed);
    const basePaths = resolveCliPaths({ env, overrides });
...
function runConfigCommand(parsed, config) {
...
function runDatabaseCommand(parsed, paths, config) {
    ...
    try {
        ...
        throw new CliError("DATABASE_QUERY_FAILED", `LCM database query failed: ${error instanceof Error ? error.message : String(error)}`, 5, { databasePath: paths.databasePath });
    }
    finally {
        db.close();
    }
...
export function runCli(args, io = {}) {
    ...
    const writeStdout = io.stdout ?? ((text) => process.stdout.write(text));
    const writeStderr = io.stderr ?? ((text) => process.stderr.write(text));
    let parsed;
    try {
        ...
        const envelope = createSuccessEnvelope(parsed.command.kind, result.data, { databasePath: paths.databasePath, configPath: paths.configPath }, result.pagination);
        writeStdout(renderSuccessEnvelope(envelope, parsed.format, parsed.pretty));
        return 0;
    }
    catch (error) {
