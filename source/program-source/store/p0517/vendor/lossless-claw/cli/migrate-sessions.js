#!/usr/bin/env node
import { defaultDbPath, defaultStateDir, runSessionMigration } from "../migrate-sessions.js";
function usage() {
    return [
...
function formatHumanSummary(result, verbose = false) {
    ...
    lines.push(`files: ${result.scannedFiles} scanned, ${result.importedFiles} imported, ${result.skippedFiles} skipped, ${result.errorFiles} errors`, `messages: ${result.importedMessages} imported`);
    if (!result.apply) {
