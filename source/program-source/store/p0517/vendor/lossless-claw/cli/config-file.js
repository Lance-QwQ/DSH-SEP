...
import { chmodSync, closeSync, constants, copyFileSync, existsSync, fsyncSync, lstatSync, openSync, readFileSync, renameSync, rmSync, writeFileSync, } from "node:fs";
...
function loadConfigSchema() {
    ...
    if (!manifestUrl) {
        throw new CliError("CONFIG_SCHEMA_NOT_FOUND", "Could not locate the packaged Lossless plugin manifest.", 4);
    }
...
function readRootConfig(configPath) {
    ...
    try {
        ...
        throw new CliError("CONFIG_PARSE_FAILED", `OpenClaw config must be strict JSON for Lossless CLI editing: ${error instanceof Error ? error.message : String(error)}`, 4, { configPath });
    }
...
function optionalRecord(parent, key, configPath) {
    ...
    if (!record) {
        throw new CliError("CONFIG_SHAPE_INVALID", `OpenClaw config property ${key} must be an object.`, 4, { configPath, key });
    }
    return record;
}
// Extract only the Lossless plugin config so unrelated OpenClaw secrets never escape.
function extractLosslessConfig(root, configPath) {
...
export function readRawLosslessConfig(configPath) {
...
function listEnvironmentOverrides(env) {
...
export function readConfigView(configPath, env = process.env) {
    ...
    try {
        ...
        throw new CliError("CONFIG_VALIDATION_FAILED", `Lossless config at ${configPath} failed runtime validation: ${error instanceof Error ? error.message : String(error)}`, 4, { configPath });
    }
    return {
        configPath,
        raw,
...
function readValueAtPath(root, segments) {
    let current = root;
...
function effectivePathSegments(segments) {
...
export function getConfigValue(view, path) {
    const segments = parseConfigPath(path);
    const raw = readValueAtPath(view.raw, segments);
    const effective = readValueAtPath(view.effective, effectivePathSegments(segments));
...
function schemaAtPath(segments) {
...
function assertNoRelevantIncludes(root, configPath) {
    const ancestors = [{ label: "root", value: root }];
    const plugins = asRecord(root.plugins);
    const entries = asRecord(plugins?.entries);
    const entry = asRecord(entries?.[LOSSLESS_PLUGIN_ID]);
    const config = asRecord(entry?.config);
    ancestors.push({ label: "plugins", value: plugins }, { label: "plugins.entries", value: entries }, { label: `plugins.entries.${LOSSLESS_PLUGIN_ID}`, value: entry }, { label: `plugins.entries.${LOSSLESS_PLUGIN_ID}.config`, value: config });
    const include = ancestors.find(({ value }) => value && Object.hasOwn(value, "$include"));
    if (include) {
        throw new CliError("CONFIG_INCLUDE_UNSUPPORTED", `Lossless CLI config writes do not flatten $include at ${include.label}.`, 4, { configPath, includePath: include.label });
    }
}
// Create one object property while rejecting existing scalar or array shapes.
function getOrCreateRecord(parent, key, configPath) {
    if (!Object.hasOwn(parent, key)) {
        parent[key] = {};
    }
    const record = asRecord(parent[key]);
    if (!record) {
        throw new CliError("CONFIG_SHAPE_INVALID", `Cannot create Lossless config below non-object property ${key}.`, 4, { configPath, key });
    }
    return record;
}
// Set one nested property after the manifest path has been validated.
function setValueAtPath(root, segments, value, configPath) {
    let parent = root;
...
function parseJsonValue(value, path) {
    try {
        ...
        throw new CliError("CONFIG_VALIDATION_FAILED", `Value for ${path} must be valid JSON: ${error instanceof Error ? error.message : String(error)}`, 4, { path });
    }
}
// Validate the complete plugin config to catch both target and surrounding invalid values.
function validatePluginConfig(config, path) {
    const errors = validateSchema(getConfigSchema(), config).slice(0, 20);
    if (errors.length === 0) {
        return;
    }
    throw new CliError("CONFIG_VALIDATION_FAILED", `Lossless config would be invalid after setting ${path}.`, 4, { path, errors });
}
// Apply runtime cross-field validation that the manifest schema cannot express.
function validateRuntimeConfig(config, path) {
    try {
        resolveLcmConfigWithDiagnostics({}, config);
    }
    catch (error) {
        throw new CliError("CONFIG_VALIDATION_FAILED", `Lossless config would fail runtime validation after setting ${path}: ${error instanceof Error ? error.message : String(error)}`, 4, { path });
    }
}
...
function fsyncParentDirectory(configPath) {
...
function writeConfigAtomically(configPath, root, mode) {
    ...
    try {
        ...
        throw new CliError("CONFIG_WRITE_FAILED", `Could not atomically replace OpenClaw config: ${error instanceof Error ? error.message : String(error)}`, 4, { configPath });
    }
}
/** Set one manifest-declared Lossless config value with backup-first atomic replacement. */
export function setConfigValue(configPath, path, jsonValue, options = {}) {
    ...
    if (stat.isSymbolicLink()) {
        throw new CliError("CONFIG_SYMLINK_UNSUPPORTED", "Lossless CLI config writes refuse symlink targets.", 4, { configPath });
    }
    ...
    if (targetErrors.length > 0) {
        throw new CliError("CONFIG_VALIDATION_FAILED", `Value does not match the manifest schema for ${path}.`, 4, { path, errors: targetErrors.slice(0, 20) });
    }
    ...
    try {
        ...
        throw new CliError("CONFIG_BACKUP_FAILED", `Could not create OpenClaw config backup: ${error instanceof Error ? error.message : String(error)}`, 4, { configPath, backupPath });
    }
