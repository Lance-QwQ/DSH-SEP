...
export function openReadOnlyDatabase(databasePath) {
    if (!existsSync(databasePath) || !statSync(databasePath).isFile()) {
        throw new CliError("DATABASE_NOT_FOUND", `LCM database not found at ${databasePath}.`, 3, { databasePath });
    }
    let database;
    try {
        ...
        throw new CliError("DATABASE_OPEN_FAILED", `Could not open LCM database: ${message}`, 5, { databasePath });
    }
    return database;
}
