...
const SUMMARY_KINDS = new Set(["leaf", "condensed"]);
...
function parsePositiveInteger(value, label, maximum) {
    ...
    if (parsed < 1 || (maximum !== undefined && parsed > maximum)) {
        throw new CliError("INVALID_ARGUMENT", `${label} must be between 1 and ${maximum ?? Number.MAX_SAFE_INTEGER}.`, 2, { value });
    }
    return parsed;
}
...
function parseDurationMilliseconds(value) {
    ...
    if (!match || Number.parseInt(match[1], 10) < 1) {
        throw new CliError("INVALID_TIME_FILTER", "recency must be a positive integer followed by s, m, h, d, or w.", 2, { value });
    }
    const unitMs = { s: 1_000, m: 60_000, h: 3_600_000, d: 86_400_000, w: 604_800_000 };
    return Number.parseInt(match[1], 10) * unitMs[match[2].toLowerCase()];
}
/** Parse and validate inclusive lower and exclusive upper CLI time filters. */
export function parseTimeFilter(input, now = new Date()) {
    if (input.between && (input.after || input.before || input.recency)) {
        throw new CliError("INVALID_TIME_FILTER", "between cannot be combined with after, before, or recency.", 2);
    }
...
function parseSelector(conversationId, sessionKey) {
...
function parseCommand(positionals, help, version) {
    if (version) {
        return { kind: "version" };
    }
...
function requireSelector(command, selector) {
    if (!selector
        && ["conversations.show", "messages.list", "messages.tail"].includes(command.kind)) {
        throw new CliError("MISSING_SELECTOR", `${command.kind} requires conversation-id or session-key.`, 2);
    }
}
// Protect one negative JSON number from option parsing when it is the config-set value.
function normalizeDashPrefixedConfigValue(args) {
    for (const [index, originalValue] of args.entries()) {
        ...
        try {
            ...
            if (inspected.positionals.length === 4
...
function assertSupportedCommandOptions(command, values) {
    const supplied = Object.keys(COMMAND_OPTION_SUPPORT)
        .filter((option) => {
    ...
    for (const option of supplied) {
        if (!COMMAND_OPTION_SUPPORT[option].has(command.kind)) {
            throw new CliError("INVALID_ARGUMENT", `--${option} is not supported by ${command.kind}.`, 2, { option, command: command.kind });
        }
    }
}
/** Parse one complete `lcm` invocation into a validated command contract. */
export function parseCliArgs(args, now = new Date()) {
    let parsed;
    try {
        parsed = parseRawCliArgs(args);
    }
    catch (error) {
        throw new CliError("INVALID_ARGUMENT", error instanceof Error ? error.message : String(error), 2);
    }
    ...
    if (summaryKindValue && !SUMMARY_KINDS.has(summaryKindValue)) {
...
export function decodeCursor(cursor, expectedResource) {
    try {
