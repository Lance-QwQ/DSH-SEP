const OPENCLAW_INBOUND_METADATA_BLOCK_RE = /^(Conversation info \(untrusted metadata\)|Sender \(untrusted metadata\)):\r?\n```json\r?\n([\s\S]*?)\r?\n```/;
...
function escapeOpenClawRecapHeaderRegExpLiteral(value) {
    return value.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
}
const OPENCLAW_INBOUND_CONTEXT_BLOCK_RE = new RegExp(`^(?:${OPENCLAW_INBOUND_CONTEXT_BLOCK_HEADINGS.map(escapeOpenClawRecapHeaderRegExpLiteral).join("|")}):` + String.raw `\r?\n\`\`\`json\r?\n([\s\S]*?)\r?\n\`\`\``);
const OPENCLAW_INBOUND_HISTORY_RECAP_HEADER_SRC = `(?:${OPENCLAW_INBOUND_HISTORY_RECAP_HEADERS.map(escapeOpenClawRecapHeaderRegExpLiteral).join("|")})`;
...
const OPENCLAW_INBOUND_HISTORY_RECAP_BLOCK_RE = new RegExp(`^${OPENCLAW_INBOUND_HISTORY_RECAP_HEADER_SRC}` +
    String.raw `\r?\n\`\`\`json\r?\n([\s\S]*?)\r?\n\`\`\``);
...
const OPENCLAW_INBOUND_HISTORY_RECAP_LINE_TIMESTAMP_PREFIX_RE = /^[A-Za-z]{3} \d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2} \S+ /;
...
function matchLeadingOpenClawInboundHistoryRecapLineBlock(candidate) {
    const header = OPENCLAW_INBOUND_HISTORY_RECAP_HEADERS.find((headerText) => candidate.startsWith(headerText));
    if (header === undefined) {
        return 0;
    }
    ...
    const terminated = rest.length === 0 ||
        rest === "\n" ||
        rest === "\r\n" ||
        rest.startsWith("\n\n") ||
        rest.startsWith("\n\r\n") ||
...
const OPENCLAW_INBOUND_TIMESTAMP_PREFIX_RE = /^\s*\[[A-Za-z]{3}\s+\d{4}-\d{2}-\d{2}[^\]]*GMT[^\]]*\]\s*/;
/**
 ...
 */
export function stripLeadingOpenClawInboundTimestamp(value) {
 ...
 * decoration.
 */
export function contentBeginsWithOpenClawInboundMetadataBlock(content) {
 ...
 */
export function extractBodyAfterOpenClawInboundMetadataBlock(content) {
...
function extractBodyAfterOpenClawInboundMetadataBlockWithPolicy(content, stripInjectedContext) {
    ...
    const firstRecord = parseOpenClawInboundMetadataRecord(firstMatch[1] ?? "", firstMatch[2] ?? "");
    if (firstRecord === null) {
        return null;
    }
 ...
 */
export function openClawInboundBodiesMatch(liveContent, bareContent) {
    const bareBody = stripLeadingOpenClawInboundTimestamp(bareContent);
    const exactLiveBodyAfterMetadata = extractBodyAfterOpenClawInboundMetadataBlockWithPolicy(liveContent, false);
 ...
 */
export function openClawInboundBodiesMatchWithInjectedContext(liveContent, bareContent) {
 ...
 */
export function openClawInboundDecoratedBodiesMatch(liveContent, assembledContent) {
    const liveBody = extractBodyAfterOpenClawInboundMetadataBlock(liveContent);
    const assembledBody = extractBodyAfterOpenClawInboundMetadataBlockWithPolicy(assembledContent, false);
    return (liveBody !== null &&
        assembledBody !== null &&
 ...
 */
export function canonicalizeOpenClawInboundMetadataIdentityContent(role, content) {
 ...
 */
export function canonicalizeOpenClawInboundMetadataIdentityContentBeforeHistoryRecap(role, content) {
    return canonicalizeOpenClawInboundMetadataIdentityContentWithRecapPolicy(role, content, false);
}
function canonicalizeOpenClawInboundMetadataIdentityContentWithRecapPolicy(role, content, stripHistoryRecap) {
    if (role !== "user") {
        return content;
    }
    ...
    const canonicalConversationJson = conversationRecord
        ? canonicalizeMetadataJson(conversationRecord, stripHistoryRecap && hasOpenClawInboundHistory(conversationRecord)
 ...
 * body.
 */
export function isOpenClawAmbientInboundRecord(role, content) {
    if (role !== "user") {
        return false;
    }
...
function parseOpenClawInboundMetadataRecord(heading, json) {
    let parsed;
    try {
...
function hasOpenClawInboundHistory(record) {
...
function stripLeadingInjectedContextTagBlocks(content) {
    let remaining = content;
    while (true) {
        const candidate = remaining.trimStart();
        const name = OPENCLAW_INJECTED_CONTEXT_TAG_NAMES.find((tag) => candidate.startsWith(`<${tag}>`));
        if (name === undefined) {
            return remaining;
        }
        const closeTag = `</${name}>`;
...
function splitLeadingOpenClawInboundContextBlocks(content) {
    let remaining = content;
    let blocksText = "";
    while (true) {
        const candidate = remaining.trimStart();
...
function isValidOpenClawInboundContextPayload(json) {
    let parsed;
    try {
        parsed = JSON.parse(json);
    }
 ...
 * entries).
 */
function isValidOpenClawInboundHistoryRecapPayload(json) {
    let parsed;
    try {
        parsed = JSON.parse(json);
    }
 ...
 */
function matchLeadingOpenClawInboundHistoryRecap(candidate) {
...
function canonicalizeMetadataJson(record, volatileKeys) {
    const stableEntries = Object.entries(record)
        .filter(([key]) => !volatileKeys.has(key))
        .map(([key, value]) => [key, canonicalizeJsonValue(value)])
...
function formatCanonicalMetadataBlock(heading, json) {
    return [heading + ":", "```json", json, "```"].join("\n");
}
function stripMetadataSeparator(content) {
    return content.replace(/^[ \t]*(?:\r?\n)(?:[ \t]*(?:\r?\n))?/, "");
}
function getKnownKeysForHeading(heading) {
