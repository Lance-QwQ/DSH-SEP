...
function firstString(bags, keys) {
...
function firstContextWindow(bags) {
    for (const bag of bags) {
 ...
 */
export function readRuntimeModelContext(...bags) {
    const present = bags.filter((bag) => bag !== undefined);
    const provider = firstString(present, PROVIDER_KEYS);
    const model = firstString(present, MODEL_KEYS);
    const modelRef = model && provider && !model.includes("/") ? `${provider}/${model}` : model;
