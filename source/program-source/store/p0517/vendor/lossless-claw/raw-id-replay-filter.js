...
import { externalizedReplayMetadataMatches, extractPlainToolReplayTextsById, extractRawBlockIdsFromPartMetadata, extractRawBlockSignatureFromPartMetadata, extractRawIdsFromPartMetadata, } from "./replay-metadata.js";
...
export async function filterPersistedRawIdReplayBatch(params) {
    ...
    const rawCoverageStmt = params.db.prepare(`SELECT m.message_id AS messageId
    ...
    const identityCoverageStmt = params.db.prepare(`SELECT m.message_id AS messageId
     FROM message_parts mp
     JOIN messages m ON m.message_id = mp.message_id
     WHERE m.conversation_id = ?
       AND m.role = ?
       ...
       AND json_valid(mp.metadata)
       AND ${idMatchPredicate}`);
    const externalizedCoverageStmt = params.db.prepare(`SELECT
     ...
     WHERE m.conversation_id = ?
       ...
       AND ${idMatchPredicate}`);
    const rawBlockSignatureStmt = params.db.prepare(`SELECT metadata
     FROM message_parts
     WHERE message_id = ?
    ...
    for (const message of params.messages) {
        ...
        const rawBlockSignatures = [];
        const replayIdsByPart = [];
        ...
        const coverageByMessageId = new Map();
        for (const rawId of matchedIds) {
            ...
            if (canMatchWithoutIdentity) {
                rows = rawCoverageStmt.all(params.conversationId, stored.role, ...rawIdArgs);
            }
            else {
                const identityHash = buildMessageIdentityHash(stored.role, stored.content);
                rows = identityCoverageStmt.all(params.conversationId, stored.role, identityHash, ...rawIdArgs);
            }
            for (const row of rows) {
                const matchedRawIds = coverageByMessageId.get(row.messageId) ?? new Set();
        ...
        if (canMatchWithoutIdentity) {
            for (const [messageId, rawIds] of coverageByMessageId.entries()) {
                ...
                const rows = rawBlockSignatureStmt.all(messageId);
                ...
                for (let index = 0; index < rows.length; index += 1) {
                    ...
                    if (persistedSignature === rawBlockSignatures[index] &&
                    ...
                    for (const rawId of replayIdsByPart[index] ?? []) {
                        ...
                        try {
                            contentMatches = await params.summaryStore.largeFileContentEquals(fileId, externalizedText, { largeFilesDir: params.largeFilesDir });
                        }
                        catch {
                            continue;
                        }
        ...
        if (!alreadyPersisted && canUseExternalizedFallback && externalizedTextsById.size > 0) {
            const externalizedCoverageByMessageId = new Map();
            for (const rawId of matchedIds) {
                const externalizedText = externalizedTextsById.get(rawId);
                if (externalizedText === undefined) {
                    continue;
                }
                const incomingPartIndex = replayIdsByPart.findIndex((partRawIds) => partRawIds.includes(rawId));
                ...
                const rows = externalizedCoverageStmt.all(params.conversationId, stored.role, ...rawIdArgs);
                for (const row of rows) {
                    ...
                    let contentMatches = false;
                    try {
                        contentMatches = await params.summaryStore.largeFileContentEquals(row.fileId, externalizedText, { largeFilesDir: params.largeFilesDir });
                    }
                    catch {
                        continue;
                    }
                    ...
                    const matchedRawIds = externalizedCoverageByMessageId.get(row.messageId) ?? new Set();
                    matchedRawIds.add(rawId);
                    externalizedCoverageByMessageId.set(row.messageId, matchedRawIds);
                }
            }
            alreadyPersisted = Array.from(externalizedCoverageByMessageId.values()).some((rawIds) => rawIds.size === matchedIds.size);
        }
        if (alreadyPersisted) {
            replayedMessages += 1;
        }
    ...
    if (replayedMessages > 0) {
        params.log.warn(`[lcm] ingestBatch: dropped ${replayedMessages}/${params.messages.length} raw-id replay messages for ${params.sessionContext}`);
    }
    return filtered;
}
