...
import { DELIBERATE_ARCHIVE_CAUSES, normalizeMessageContentForFullTextIndex, } from "../store/conversation-store.js";
...
function countsFromRows(rows) {
    return rows.reduce((counts, row) => addCounts(counts, {
...
function isDeliberateArchive(row) {
    return (row.archive_cause !== null && DELIBERATE_ARCHIVE_CAUSES.has(row.archive_cause));
}
function compareConversationChronology(left, right) {
...
function loadSessionKeysWithMultipleConversations(db) {
    const rows = db
...
function loadConversationRows(db, sessionKey) {
    return db
        .prepare(`SELECT
         c.conversation_id,
         c.session_key,
...
function loadUnhandledConversationIdTables(db) {
    const tables = db
        .prepare(`SELECT name
       FROM sqlite_master
       WHERE type = 'table'
...
function loadDuplicateRows(db, sql, conversationIds) {
    if (conversationIds.length === 0)
        return [];
    return db.prepare(sql.replace("__IDS__", placeholders(conversationIds))).all(...conversationIds);
}
function loadCollisionReasons(db, conversationIds) {
    const duplicateTranscriptEntries = loadDuplicateRows(db, `SELECT transcript_entry_id AS value, COUNT(*) AS count
     FROM messages
     WHERE conversation_id IN (__IDS__)
       AND transcript_entry_id IS NOT NULL
     GROUP BY transcript_entry_id
     HAVING COUNT(*) > 1`, conversationIds);
    const duplicateSummaries = loadDuplicateRows(db, `SELECT summary_id AS value, COUNT(*) AS count
     FROM summaries
     WHERE conversation_id IN (__IDS__)
     GROUP BY summary_id
     HAVING COUNT(*) > 1`, conversationIds);
    const duplicateFiles = loadDuplicateRows(db, `SELECT file_id AS value, COUNT(*) AS count
     FROM large_files
     WHERE conversation_id IN (__IDS__)
     GROUP BY file_id
     HAVING COUNT(*) > 1`, conversationIds);
...
function classifySessionKeyGroup(params) {
    ...
    const sources = rows.filter((row) => row.active === 0 && hasStrandedData(row) && !isDeliberateArchive(row));
    if (sources.length === 0) {
        return null;
    }
 ...
 */
export function scanRolloverSplits(db) {
...
function loadOrderedMessageIds(db, group) {
    ...
    const rows = db
       ...
       ORDER BY CASE conversation_id ${conversationRankSql} ELSE 999999 END,
                seq ASC,
                message_id ASC`)
        .all(...group.orderedConversationIds);
    return rows.map((row) => row.message_id);
}
function loadOrderedContextRowIds(db, group) {
    const conversationRankSql = group.orderedConversationIds
        .map((conversationId, index) => `WHEN ${conversationId} THEN ${index}`)
        .join(" ");
    const rows = db
        .prepare(`SELECT rowid AS row_id
       FROM context_items
       WHERE conversation_id IN (${placeholders(group.orderedConversationIds)})
       ORDER BY CASE conversation_id ${conversationRankSql} ELSE 999999 END,
                ordinal ASC,
...
function reparentMessages(db, group) {
    const messageIds = loadOrderedMessageIds(db, group);
    const stage = db.prepare(`UPDATE messages
...
function reparentContextItems(db, group) {
    const rowIds = loadOrderedContextRowIds(db, group);
    const stage = db.prepare(`UPDATE context_items
...
function reparentSimpleConversationTables(db, group) {
    ...
    for (const table of ["summaries", "large_files", "focus_briefs"]) {
        db.prepare(`UPDATE ${quoteSqlIdentifier(table)}
       SET conversation_id = ?
       WHERE conversation_id IN (${sourcePlaceholders})`).run(group.targetConversationId, ...group.sourceConversationIds);
    }
}
...
function getTranscriptEpochModeRank(mode) {
    return TRANSCRIPT_EPOCH_MODE_RANK.get(mode) ?? TRANSCRIPT_EPOCH_MODE_RANK.get("corrupt");
}
function reparentTranscriptAnchorState(db, group) {
    if (group.sourceConversationIds.length === 0) {
        return;
    }
    const sourcePlaceholders = placeholders(group.sourceConversationIds);
    if (hasTable(db, "message_transcript_anchor_trust")) {
        db.prepare(`UPDATE message_transcript_anchor_trust
       SET conversation_id = ?,
    ...
    const epochRows = db
        .prepare(`SELECT conversation_id, migration_mode
    ...
    const worstMode = epochRows.reduce((mode, row) => getTranscriptEpochModeRank(row.migration_mode) > getTranscriptEpochModeRank(mode)
        ? row.migration_mode
        : mode, "verified");
    const targetEpoch = epochRows.find((row) => row.conversation_id === group.targetConversationId);
    db.prepare(`DELETE FROM conversation_transcript_epochs
     WHERE conversation_id IN (${sourcePlaceholders})`).run(...group.sourceConversationIds);
    ...
    if (targetEpoch) {
        if (getTranscriptEpochModeRank(worstMode) > getTranscriptEpochModeRank(targetEpoch.migration_mode)) {
            db.prepare(`UPDATE conversation_transcript_epochs
         SET migration_mode = ?,
             metadata_json = ?,
             updated_at = datetime('now')
         WHERE conversation_id = ?`).run(worstMode, mergedMetadata, group.targetConversationId);
        }
        return;
    }
    const targetConversation = db
        .prepare(`SELECT session_id, session_key
       FROM conversations
       WHERE conversation_id = ?`)
        .get(group.targetConversationId);
    if (!targetConversation) {
        throw new Error("rollover split target conversation disappeared during transcript epoch repair");
    }
    db.prepare(`INSERT INTO conversation_transcript_epochs (
       conversation_id,
       session_id,
       session_key,
       migration_mode,
       metadata_json
     ) VALUES (?, ?, ?, ?, ?)`).run(group.targetConversationId, targetConversation.session_id, targetConversation.session_key, worstMode, mergedMetadata);
}
...
function deleteInvalidatedPendingCompactionState(db, group) {
    if (!hasTable(db, "pending_compaction_batches")) {
        return;
    }
    const idPlaceholders = placeholders(group.orderedConversationIds);
    db.prepare(`DELETE FROM pending_summary_nodes
     WHERE conversation_id IN (${idPlaceholders})`).run(...group.orderedConversationIds);
    db.prepare(`DELETE FROM pending_compaction_batches
     WHERE conversation_id IN (${idPlaceholders})`).run(...group.orderedConversationIds);
}
function clearSourceStateAndMarkTarget(db, group) {
    const sourcePlaceholders = placeholders(group.sourceConversationIds);
    for (const table of [
        "conversation_bootstrap_state",
        "conversation_compaction_maintenance",
        ...
        db.prepare(`DELETE FROM ${quoteSqlIdentifier(table)}
       WHERE conversation_id IN (${sourcePlaceholders})`).run(...group.sourceConversationIds);
    }
    db.prepare(`UPDATE conversations
     SET created_at = (
         ...
         updated_at = datetime('now')
     WHERE conversation_id = ?`).run(...group.orderedConversationIds, group.targetConversationId);
    db.prepare(`INSERT INTO conversation_compaction_maintenance (
       conversation_id,
       pending,
       requested_at,
       reason,
     ...
     ON CONFLICT(conversation_id) DO UPDATE SET
       ...
       updated_at = datetime('now')`).run(group.targetConversationId, ROLLOVER_SPLIT_MAINTENANCE_REASON);
}
function rebuildFtsTables(db) {
...
function hasNewForeignKeyViolations(status) {
    return (status !== "clean" &&
...
function assertNoRows(db, sql, params, message) {
    const rows = db.prepare(sql).all(...params);
    if (rows.length > 0) {
        throw new Error(message);
    }
}
function verifyRepairedTargets(db, targetConversationIds) {
    if (targetConversationIds.length === 0) {
        return;
    }
    const ids = placeholders(targetConversationIds);
    assertNoRows(db, `SELECT conversation_id
     FROM messages
     WHERE conversation_id IN (${ids})
     GROUP BY conversation_id
     HAVING COUNT(*) > 0
        AND (MIN(seq) != 1 OR MAX(seq) != COUNT(*) OR COUNT(DISTINCT seq) != COUNT(*))`, targetConversationIds, "message seq verification failed");
    assertNoRows(db, `SELECT conversation_id
     FROM context_items
     WHERE conversation_id IN (${ids})
     GROUP BY conversation_id
     HAVING COUNT(*) > 0
        AND (MIN(ordinal) != 0 OR MAX(ordinal) != COUNT(*) - 1 OR COUNT(DISTINCT ordinal) != COUNT(*))`, targetConversationIds, "context ordinal verification failed");
    assertNoRows(db, `SELECT ci.conversation_id
     FROM context_items ci
     JOIN messages m ON m.message_id = ci.message_id
     WHERE ci.conversation_id IN (${ids})
       AND ci.item_type = 'message'
       AND m.conversation_id != ci.conversation_id`, targetConversationIds, "context message reference verification failed");
    assertNoRows(db, `SELECT ci.conversation_id
     FROM context_items ci
     JOIN summaries s ON s.summary_id = ci.summary_id
     WHERE ci.conversation_id IN (${ids})
       AND ci.item_type = 'summary'
       AND s.conversation_id != ci.conversation_id`, targetConversationIds, "context summary reference verification failed");
}
function repairSafeGroup(db, group) {
...
export function getRolloverSplitApplyUnavailableReason(databasePath) {
...
export async function applyRolloverSplitRepair(params) {
    ...
    await withDatabaseTransaction(params.db, "BEGIN IMMEDIATE", () => {
        ...
        verifyRepairedTargets(params.db, safeGroups.map((group) => group.targetConversationId));
