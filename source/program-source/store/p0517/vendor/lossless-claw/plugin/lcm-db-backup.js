...
export function buildLcmDatabaseBackupPath(databasePath, label) {
    ...
    return join(dirname(fileBackedDatabasePath), `${basename(fileBackedDatabasePath)}.${normalizeBackupLabel(label)}-${timestamp}-${suffix}.bak`);
}
export function buildLcmDatabaseLatestBackupPath(databasePath, label) {
    const fileBackedDatabasePath = getFileBackedDatabasePath(databasePath);
    if (!fileBackedDatabasePath) {
        return null;
    }
    return join(dirname(fileBackedDatabasePath), `${basename(fileBackedDatabasePath)}.${normalizeBackupLabel(label)}-latest.bak`);
}
export function writeLcmDatabaseBackup(db, backupPath) {
    db.exec(`VACUUM INTO ${quoteSqlString(backupPath)}`);
}
export function createLcmDatabaseBackup(db, options) {
