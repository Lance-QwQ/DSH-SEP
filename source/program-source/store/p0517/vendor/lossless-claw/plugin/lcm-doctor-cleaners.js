...
const CLEANER_DEFINITIONS = [
    ...
    },
    {
        ...
        description: "Archived conversations with NULL session_key whose first stored message begins with [Subagent Context].",
        candidatePredicateSql: "(c.session_key IS NULL AND c.active = 0 AND c.archived_at IS NOT NULL)",
        predicateSql: "(c.session_key IS NULL AND c.active = 0 AND c.archived_at IS NOT NULL AND message_stats.first_message_preview LIKE '[Subagent Context]%')",
        needsFirstMessage: true,
    },
];
const DOCTOR_CLEANER_IDS = CLEANER_DEFINITIONS.map((definition) => definition.id);
function getCleanerDefinitions(filterIds) {
...
function buildCleanerPredicateSql(definition, kind) {
...
function stageCleanerAgentIds(db, agentIds) {
    db.exec(`
    CREATE TEMP TABLE doctor_cleaner_agent_ids (
      agent_id TEXT PRIMARY KEY
    ) WITHOUT ROWID
  `);
    const insert = db.prepare(`INSERT INTO temp.doctor_cleaner_agent_ids (agent_id) VALUES (?)`);
...
function buildMatchedConversationsSql(params) {
    ...
    return definitions
        ...
        const joinSql = definition.needsFirstMessage && messageStatsTableName
...
function buildCandidateConversationsSql(definitions) {
    if (definitions.length === 0) {
        return `SELECT NULL AS conversation_id WHERE 0`;
    }
    return definitions
        .map((definition) => `SELECT c.conversation_id
              FROM conversations c
              WHERE ${buildCleanerPredicateSql(definition, "candidate")}`)
        .join(`\nUNION\n`);
}
function dropTempCleanerScanTables(db) {
...
function stageCleanerScanTables(db, definitions, agentIds) {
...
export function getDoctorCleanerFilters() {
...
export function getDoctorCleanerFilterIds() {
    return [...DOCTOR_CLEANER_IDS];
}
export function scanDoctorCleaners(db, filterIds, agentIds) {
    ...
    try {
        ...
        const counts = db
         ...
         FROM filter_counts fc`)
            .all();
        const examples = db
            .prepare(`WITH ranked_examples AS (
           SELECT
             matches.filter_id,
             c.conversation_id,
             c.session_key,
...
function dropTempCleanerTables(db) {
...
function stageTempCleanerFirstMessages(db) {
    db.exec(`
...
function stageCleanerConversationIds(db, definitions, agentIds) {
...
function readTempCleanerDeleteCounts(db) {
    const row = db
        .prepare(`SELECT
...
function deleteTempCleanerCandidates(db) {
    ...
    db.prepare(`DELETE FROM summary_messages
     WHERE summary_id IN (SELECT summary_id FROM temp.doctor_cleaner_summary_ids)`).run();
    db.prepare(`DELETE FROM summary_messages
     WHERE message_id IN (SELECT message_id FROM temp.doctor_cleaner_message_ids)`).run();
    db.prepare(`DELETE FROM summary_parents
     WHERE summary_id IN (SELECT summary_id FROM temp.doctor_cleaner_summary_ids)`).run();
    db.prepare(`DELETE FROM summary_parents
     WHERE parent_summary_id IN (SELECT summary_id FROM temp.doctor_cleaner_summary_ids)`).run();
    db.prepare(`DELETE FROM context_items
     WHERE message_id IN (SELECT message_id FROM temp.doctor_cleaner_message_ids)`).run();
    db.prepare(`DELETE FROM context_items
     WHERE summary_id IN (SELECT summary_id FROM temp.doctor_cleaner_summary_ids)`).run();
    db.prepare(`DELETE FROM context_items
     WHERE conversation_id IN (SELECT conversation_id FROM temp.doctor_cleaner_conversation_ids)`).run();
    if (hasMessagesFts) {
        db.prepare(`DELETE FROM messages_fts
       WHERE rowid IN (SELECT message_id FROM temp.doctor_cleaner_message_ids)`).run();
    }
    if (hasSummariesFts) {
        db.prepare(`DELETE FROM summaries_fts
       WHERE summary_id IN (SELECT summary_id FROM temp.doctor_cleaner_summary_ids)`).run();
    }
    if (hasSummariesFtsCjk) {
        db.prepare(`DELETE FROM summaries_fts_cjk
       WHERE summary_id IN (SELECT summary_id FROM temp.doctor_cleaner_summary_ids)`).run();
    }
    return Number(db
        .prepare(`DELETE FROM conversations
         WHERE conversation_id IN (SELECT conversation_id FROM temp.doctor_cleaner_conversation_ids)`)
        .run().changes ?? 0);
}
export function getDoctorCleanerApplyUnavailableReason(databasePath) {
...
function buildCleanerBackupPath(databasePath) {
    return buildLcmDatabaseBackupPath(databasePath, "doctor-cleaners");
}
export function applyDoctorCleaners(db, options) {
    ...
    if (!backupPath) {
        return {
            kind: "unavailable",
            reason: getDoctorCleanerApplyUnavailableReason(options.databasePath)
