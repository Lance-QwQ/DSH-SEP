...
let openClawRedactor = loadOpenClawRedactor();
...
function defaultRollingPath(date = new Date()) {
...
function isRollingLogSegmentPath(file) {
    const base = path.basename(file);
...
function expandHomePrefix(file) {
    if (file === "~") {
        return os.homedir();
    }
    if (file.startsWith("~/")) {
...
function isTrustedExistingOpenClawTmpDir(stat, uid) {
    if (!stat.isDirectory() || stat.isSymbolicLink()) {
        return false;
    }
...
function getCurrentRegularLogFileBytes(file) {
    try {
...
function isExistingRegularFile(file) {
    try {
        const stat = fs.lstatSync(file);
...
function loadOpenClawRedactor() {
    try {
        ...
        if (typeof loggingCore.redactSensitiveText === "function") {
            return loggingCore.redactSensitiveText;
        }
    }
    catch {
...
function redactionPattern(rawPattern) {
    try {
...
function fallbackRedactSensitiveText(value, redaction) {
...
function redactSensitiveText(value, redaction) {
    return (openClawRedactor ?? fallbackRedactSensitiveText)(value, redaction);
}
function appendRegularFileSync(file, content) {
    let fd;
    try {
...
export function createIndependentLcmFileLogger(config, redaction) {
    if (!config.enabled) {
        return undefined;
    }
    let configuredFile;
