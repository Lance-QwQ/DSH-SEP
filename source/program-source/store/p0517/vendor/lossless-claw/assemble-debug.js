...
export function normalizeDebugTextSnippet(value, maxLength = 48) {
...
export function summarizeMessageContentShape(content) {
...
export function summarizeMessageForPrefixDebug(message) {
    ...
    const toolName = safeString(topLevel.toolName) ??
    ...
    summaryParts.push(`hash=${createHash("sha256").update(serialized).digest("hex").slice(0, 8)}`);
    return summaryParts.join("|");
}
export function describeAssembledPrefixChange(previous, messages) {
...
export function shouldLogOverflowDiagnostics(params) {
    ...
    return (params.diagnostics.totalContextTokens > budget ||
