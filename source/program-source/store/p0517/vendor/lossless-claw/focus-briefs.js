...
import { clearDelegatedExpansionContext, resolveExpansionRequestId, stampDelegatedExpansionContext, } from "./tools/lcm-expansion-recursion-guard.js";
...
function collectFocusFailureText(value, output, depth = 0) {
...
function formatFocusGatewayFailure(value, fallback) {
    const parts = [];
    collectFocusFailureText(value, parts);
    const message = parts.join(" ").replace(/\s+/g, " ").trim();
    return message || fallback;
}
function normalizeExpansionPrompts(value) {
    if (!Array.isArray(value)) {
        return [];
    }
    const output = [];
...
function mergeExpansionPrompts(left, right) {
    const seen = new Set();
    const output = [];
...
function parseFocusBriefReply(rawReply) {
    ...
    for (const candidate of candidates) {
        try {
            ...
            const briefMarkdown = typeof parsed.briefMarkdown === "string"
...
function parseFocusEvidenceReply(rawReply) {
    const reply = rawReply?.trim();
    if (!reply) {
    ...
    for (const candidate of candidates) {
        try {
            const parsed = JSON.parse(candidate);
            const evidenceMarkdown = typeof parsed.evidenceMarkdown === "string" ? parsed.evidenceMarkdown.trim() : "";
            if (!evidenceMarkdown) {
...
function buildPersistedFocusResultJson(parsed, truncated, warning) {
    if (!parsed.rawResultJson || (!truncated && !warning)) {
        return parsed.rawResultJson;
    }
    try {
        const raw = JSON.parse(parsed.rawResultJson);
        if (truncated) {
            raw.truncated = true;
        }
        if (warning) {
...
function buildActiveSummaryManifest(summaries) {
    return summaries
        .map((summary) => {
...
function resolveFocusTargetTokens(summaryTokens) {
    ...
    return Math.min(MAX_FOCUS_BRIEF_TARGET_TOKENS, Math.max(MIN_FOCUS_BRIEF_TARGET_TOKENS, expandedTarget));
}
function resolveFocusMinimumTokens(targetTokens) {
...
function resolveFocusDelegationTimeoutMs(params) {
    ...
    return Math.min(MAX_FOCUS_BRIEF_TIMEOUT_MS, Math.max(configured, MIN_FOCUS_BRIEF_TIMEOUT_MS, targetDerived));
}
...
async function runFocusAttempt(params) {
    let runId = "";
    try {
        ...
        if (!runId) {
            return {
                status: "error",
                runId,
                error: formatFocusGatewayFailure(response?.error ?? response, `delegated ${params.phaseName} did not return a runId`),
            };
        }
        ...
        const rawReply = params.deps.readLatestAssistantReply(Array.isArray(replyPayload.messages) ? replyPayload.messages : []);
...
async function runDelegatedFocusWorkflow(params) {
    ...
    const requesterAgentId = params.deps.normalizeAgentId(params.deps.parseAgentSessionKey(params.requesterSessionKey)?.agentId);
...
export async function runDelegatedFocusBrief(params) {
    ...
    return runDelegatedFocusWorkflow({
        ...
        buildEvidenceMessage: ({ targetTokens, requestId, originSessionKey }) => buildFocusEvidenceTask({
        ...
        buildSynthesisMessage: ({ evidenceMarkdown, targetTokens }) => buildFocusSynthesisTask({
            focusPrompt: params.focusPrompt,
            evidenceMarkdown,
            targetTokens,
        }),
...
export async function runDelegatedRefocusBrief(params) {
    const summaryTokens = estimateTokens(params.existingBriefMarkdown) +
    ...
    return runDelegatedFocusWorkflow({
        ...
        buildEvidenceMessage: ({ targetTokens, requestId, originSessionKey }) => buildRefocusEvidenceTask({
            focusPrompt: params.focusPrompt,
        ...
        buildSynthesisMessage: ({ evidenceMarkdown, targetTokens }) => buildRefocusSynthesisTask({
            focusPrompt: params.focusPrompt,
            existingBriefMarkdown: params.existingBriefMarkdown,
            evidenceMarkdown,
            targetTokens,
