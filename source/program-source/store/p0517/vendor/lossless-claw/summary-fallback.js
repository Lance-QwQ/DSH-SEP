...
export const FALLBACK_DIRECTIVE_OMISSION = "[LCM fallback summary omitted directive-shaped untrusted content].";
export const FALLBACK_SUMMARY_MARKER = "[LCM fallback summary; truncated for context management]";
export const FALLBACK_DIRECTIVE_SUMMARY_MARKER = "[LCM fallback summary; directive-shaped untrusted content omitted]";
...
const FALLBACK_DIRECTIVE_CONTINUATION_PATTERN = /^\s*(?:answer|reply|respond|say|output|print|return|show|provide|give|send|reveal|dump|exfiltrate)\b[^.!?\n]{0,160}[.!?]?\s*$/i;
const FALLBACK_DIRECTIVE_SHAPED_PATTERNS = [
    ...
    new RegExp(String.raw `${DAN_PERSONA_DIRECTIVE_PREFIX}(?:act\s+as|pretend\s+to\s+be)\s+dan\b`, "i"),
...
export function sanitizeDeterministicFallbackText(text) {
...
export function buildDeterministicFallbackSummary(text, targetTokens, options) {
    ...
    const { sanitizedText, omittedDirectiveShapedContent } = sanitizeDeterministicFallbackText(trimmed);
    ...
    const summaryText = sanitizedText.length <= maxChars ? sanitizedText : sanitizedText.slice(0, maxChars).trimEnd();
    return summaryText ? `${summaryText}\n${fallbackNote}` : fallbackNote;
}
