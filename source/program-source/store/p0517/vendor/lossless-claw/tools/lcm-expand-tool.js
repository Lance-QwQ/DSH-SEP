...
import { getRuntimeExpansionAuthManager, resolveDelegatedExpansionGrantId, wrapWithAuth, } from "../expansion-auth.js";
...
import { normalizeSummaryIds, runDelegatedExpansionLoop, } from "./lcm-expand-tool.delegation.js";
const LcmExpandSchema = Type.Object({
    ...
    conversationId: Type.Optional(Type.Number({
        description: "Conversation ID to scope the expansion to. If omitted, uses the current session's conversation.",
    })),
    allConversations: Type.Optional(Type.Boolean({
        description: "Set true to explicitly allow cross-conversation expansion. Ignored when conversationId is provided.",
    })),
});
function makeEmptyExpansionResult() {
    return {
        expansions: [],
        citedIds: [],
        totalTokens: 0,
 ...
 */
function toDelegatedRunReferences(delegated) {
    if (!delegated) {
        return undefined;
    }
...
export function createLcmExpandTool(input) {
    return {
        ...
        async execute(_toolCallId, params) {
            ...
            const tokenCap = typeof requestedTokenCap === "number" && Number.isFinite(requestedTokenCap)
            ...
            const delegatedGrantId = isDelegatedSession
                ...
                : undefined;
            const delegatedGrant = delegatedGrantId !== undefined ? runtimeAuthManager.getGrant(delegatedGrantId) : null;
            const authorizedOrchestrator = delegatedGrantId !== undefined ? wrapWithAuth(orchestrator, runtimeAuthManager) : null;
            ...
            if (isDelegatedSession && conversationScope.conversationId == null) {
                return jsonResult({
                    error: "lcm_expand requires a single delegated conversation scope. Provide conversationId within the delegated grant.",
                });
            }
            ...
            const resolvedConversationId = conversationScope.conversationId ??
            ...
            if (query) {
                try {
                    ...
                    const canDelegate = matchedSummaryIds.length > 0 &&
                        policy.action === "delegate_traversal" &&
                        !isDelegatedSession &&
                        !!sessionKey;
                    const delegated = canDelegate && resolvedConversationId != null
                    ...
                    const result = matchedSummaryIds.length === 0
                    ...
                    return {
                        ...
                        details: {
                            ...
                            policy,
                            executionPath,
                            delegated: delegated && delegated.status !== "ok"
                                ? {
                ...
                try {
                    if (conversationScope.conversationId != null) {
                        ...
                        if (outOfScope.length > 0) {
                            return jsonResult({
                                error: `Some summaryIds are outside conversation ${conversationScope.conversationId}: ` +
                                    outOfScope.join(", "),
                    ...
                    const canDelegate = normalizedSummaryIds.length > 0 &&
                        policy.action === "delegate_traversal" &&
                        !isDelegatedSession &&
                        !!sessionKey &&
