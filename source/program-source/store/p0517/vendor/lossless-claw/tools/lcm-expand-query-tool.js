...
import { formatExpansionFailure, runDelegatedExpandQuery, } from "./lcm-expand-query-delegation.js";
import { allocateExpansionTokenCaps, createExpansionDeadline, } from "./lcm-expansion-deadline.js";
import { normalizeSummaryIds, resolveRequesterConversationScopeId, } from "./lcm-expand-tool.delegation.js";
import { acquireExpansionConcurrencySlot, evaluateExpansionRecursionGuard, recordExpansionDelegationTelemetry, releaseExpansionConcurrencySlot, resolveExpansionRequestId, resolveNextExpansionDepth, } from "./lcm-expansion-recursion-guard.js";
...
function clampPositiveTimeoutMs(value) {
    return Math.max(1, Math.min(MAX_DYNAMIC_TOOL_TIMEOUT_MS, Math.floor(value)));
}
function resolveAdvertisedDynamicToolTimeoutMs(delegatedWaitTimeoutMs) {
    return clampPositiveTimeoutMs(delegatedWaitTimeoutMs + DYNAMIC_TOOL_TIMEOUT_HEADROOM_MS);
}
function createLcmExpandQuerySchema(dynamicToolTimeoutMs) {
    return Type.Object({
        ...
        query: Type.Optional(Type.String({
            description: "FTS5 query used to find summaries via the same full-text search path as lcm_grep before expansion. Use 1-3 distinctive terms or a quoted phrase; FTS5 defaults to AND matching, so extra terms make matches stricter. Required when summaryIds is not provided.",
        })),
        prompt: Type.String({
            description: "Natural-language question or task to answer using expanded context. Put the answer request here, not in query.",
        }),
        conversationId: Type.Optional(Type.Number({
            description: "Physical conversation ID to scope expansion to. If omitted, uses the current session family.",
        })),
        allConversations: Type.Optional(Type.Boolean({
            description: "Set true to explicitly allow cross-conversation lookup. Ignored when conversationId is provided.",
        })),
        ...
        tokenCap: Type.Optional(Type.Number({
            description: "Expansion retrieval token budget across all delegated lcm_expand calls for this query.",
            minimum: 1,
        })),
        timeoutMs: Type.Number({
            description: "Total OpenClaw dynamic tool RPC timeout in milliseconds. Use the default value unless the user asks for a shorter recall attempt; this keeps delegated recall open before the host watchdog fires.",
...
function buildConversationBuckets(candidates) {
    const buckets = new Map();
    for (const candidate of candidates) {
        const bucket = buckets.get(candidate.conversationId) ??
            {
                conversationId: candidate.conversationId,
                summaryIds: [],
                messageBackedSummaryIds: [],
                summaryIdSet: new Set(),
                explicitSummaryIdSet: new Set(),
                messageBackedSummaryIdSet: new Set(),
        ...
        if (candidate.requiresMessageExpansion &&
...
function compareConversationBuckets(left, right) {
    ...
    const recencyDelta = (right.newestMatchAt?.getTime() ?? 0) - (left.newestMatchAt?.getTime() ?? 0);
    if (recencyDelta !== 0) {
        return recencyDelta;
    }
...
function buildConversationBreakdown(results) {
...
function buildDelegatedFailureReply(results) {
...
function synthesizeConversationAnswers(params) {
    const successfulResults = params.results.filter((result) => result.status === "success");
    const failedResults = params.results.filter((result) => result.status === "failed");
    const skippedResults = params.results.filter((result) => result.status === "skipped");
    ...
    if (failedResults.length > 0) {
        notes.push(`failed conversations: ${failedResults
            .map((result) => `${result.conversationId} (${result.error})`)
            .join("; ")}`);
    }
    if (skippedResults.length > 0) {
        notes.push(`skipped conversations: ${skippedResults
            .map((result) => `${result.conversationId} (${result.error})`)
            .join("; ")}`);
    }
    if (notes.length > 0) {
...
function resolveSourceConversationId(params) {
    if (typeof params.scopedConversationId === "number") {
        ...
        if (mismatched.length > 0) {
            throw new Error(`Some summaryIds are outside conversation ${params.scopedConversationId}: ${mismatched.join(", ")}`);
        }
        return params.scopedConversationId;
    }
    const conversationIds = Array.from(new Set(params.candidates.map((candidate) => candidate.conversationId)));
    ...
    if (allowedConversationIds.size > 0) {
        ...
        if (outOfScope.length > 0) {
            throw new Error(`Some summaryIds are outside the allowed conversation scope: ${outOfScope.join(", ")}`);
        }
    }
    if (allowedConversationIds.size > 1) {
        const firstAllowed = params.candidates.find((candidate) => allowedConversationIds.has(candidate.conversationId));
        if (firstAllowed) {
    ...
    if (params.allConversations && conversationIds.length > 1) {
        throw new Error("Query matched summaries from multiple conversations. Provide conversationId or narrow the query.");
    }
    throw new Error("Unable to resolve a single conversation scope. Provide conversationId or set a narrower summary scope.");
}
function selectSingleConversationBucket(params) {
    const bucket = params.buckets.find((candidateBucket) => candidateBucket.conversationId === params.sourceConversationId);
    if (!bucket || bucket.summaryIds.length === 0) {
        throw new Error("No summaryIds available after applying conversation scope.");
    }
    return bucket;
}
function upsertSummaryCandidate(candidates, candidate) {
...
async function resolveSummaryCandidates(params) {
    ...
    if (params.query) {
        ...
        const fallbackConversationIds = Array.from(new Set((params.conversationIds && params.conversationIds.length > 0
        ...
        if (grepResult.summaries.length === 0 && fallbackConversationIds.length > 0) {
            const maxDepths = await Promise.all(fallbackConversationIds.map(async (conversationId) => ({
                conversationId,
                maxDepth: await summaryStore.getConversationMaxSummaryDepth(conversationId),
            })));
            const allowMessageFallback = maxDepths.every(({ maxDepth }) => typeof maxDepth === "number" && maxDepth <= 1);
            if (allowMessageFallback) {
                ...
                const leafLinksPerConversation = await Promise.all(Array.from(messageIdsByConversationId.entries()).map(async ([conversationId, messageIds]) => summaryStore.getLeafSummaryLinksForMessageIds(conversationId, messageIds)));
                const leafLinks = leafLinksPerConversation.flat();
                const messageConversationById = new Map(messageResult.messages.map((message) => [message.messageId, message.conversationId]));
...
export function createLcmExpandQueryTool(input) {
    const configuredDelegatedWaitTimeoutMs = input.deps.config.delegationTimeoutMs || DEFAULT_DELEGATED_WAIT_TIMEOUT_MS;
    const advertisedDynamicToolTimeoutMs = resolveAdvertisedDynamicToolTimeoutMs(configuredDelegatedWaitTimeoutMs);
    return {
        name: "lcm_expand_query",
        label: "LCM Expand Query",
        description: "Answer a focused natural-language question using delegated LCM expansion. " +
        ...
        async execute(_toolCallId, params) {
            ...
            const p = params;
            const explicitSummaryIds = normalizeSummaryIds(p.summaryIds);
            const query = typeof p.query === "string" ? p.query.trim() : "";
            const prompt = typeof p.prompt === "string" ? p.prompt.trim() : "";
            const requestedMaxTokens = typeof p.maxTokens === "number" ? Math.trunc(p.maxTokens) : undefined;
            const maxTokens = typeof requestedMaxTokens === "number" && Number.isFinite(requestedMaxTokens)
            ...
            const expansionTokenCap = typeof requestedTokenCap === "number" && Number.isFinite(requestedTokenCap)
                ? Math.max(1, requestedTokenCap)
                : Math.max(1, Math.trunc(input.deps.config.maxExpandTokens));
            const requestedDynamicToolTimeoutMs = typeof p.timeoutMs === "number" && Number.isFinite(p.timeoutMs)
            ...
            const delegatedWaitTimeoutMs = Math.max(1, Math.floor(deadline.workDeadlineMs - deadline.startedAtMs));
            ...
            const callerSessionKey = (typeof input.requesterSessionKey === "string"
            ...
            try {
                ...
                const familyScopedConversationId = (conversationScope.conversationIds?.length ?? 0) > 1
                    ? undefined
                ...
                if (candidates.length === 0) {
                    ...
                    return jsonResult(buildExpandQueryReply({
                ...
                const requesterAgentId = input.deps.normalizeAgentId(input.deps.parseAgentSessionKey(callerSessionKey)?.agentId);
                ...
                if (!conversationScope.allConversations) {
                    ...
                    if (delegatedOutcome.status === "failed") {
                        return jsonResult(buildDelegatedFailureReply([
                ...
                const delegatedOutcomes = await Promise.all(runnableBuckets.map((bucket, index) => runDelegatedExpandQuery({
                    deps: input.deps,
                    callerSessionKey,
                    requesterAgentId,
                    bucket,
                ...
                const bucketResults = delegatedOutcomes.map((outcome, index) => ({
                    ...outcome,
                ...
                const successfulResults = bucketResults.filter((result) => result.status === "success");
                ...
                return jsonResult(buildExpandQueryReply({
                    ...
                    expandedSummaryCount: successfulResults.reduce((total, result) => total + result.reply.expandedSummaryCount, 0),
                    totalSourceTokens: successfulResults.reduce((total, result) => total + result.reply.totalSourceTokens, 0),
                    truncated: successfulResults.some((result) => result.reply.truncated)
