...
async function lookupConversationForSession(input) {
    const store = input.lcm.getConversationStore();
...
export async function resolveLcmConversationScope(input) {
    ...
    const sessionIdAsSessionKey = !explicitSessionKey
    ...
    const isDelegatedSession = Boolean(normalizedSessionKey) && Boolean(input.deps?.isSubagentSessionKey(normalizedSessionKey));
    ...
    if (isDelegatedSession) {
        ...
        const delegatedGrant = delegatedGrantId != null
        ...
        if (!delegatedGrant) {
            ...
            return {
                allConversations: false,
                delegated: true,
                error: "Delegated LCM retrieval requires a valid grant. This sub-agent session has no propagated expansion grant.",
            };
        }
        allowedConversationIds = Array.from(new Set(delegatedGrant.allowedConversationIds
    ...
    if (normalizedSessionKey) {
        ...
        if (bySessionKey) {
            const byRuntimeSession = !isDelegatedSession && normalizedInputSessionId && !sessionIdAsSessionKey
                ? await store.getConversationBySessionId(normalizedInputSessionId)
                : null;
            const runtimeSessionOverridesStaleKey = byRuntimeSession?.active === true
            ...
            const familyIds = isolateResolvedSessionFamily
                ...
                : typeof lcm.getConversationStore().getConversationFamilyIds === "function"
                    ? await lcm.getConversationStore().getConversationFamilyIds({
    ...
    if (!normalizedSessionId && !input.sessionKey?.trim()) {
        return {
            conversationId: isDelegatedSession && allowedConversationIds.length === 1
