...
import { clearDelegatedExpansionContext, evaluateExpansionRecursionGuard, recordExpansionDelegationTelemetry, resolveExpansionRequestId, stampDelegatedExpansionContext, } from "./lcm-expansion-recursion-guard.js";
const MAX_GATEWAY_TIMEOUT_MS = 2_147_483_647;
export function normalizeSummaryIds(input) {
...
function parseDelegatedExpansionReply(rawReply) {
    const fallback = {
    ...
    for (const candidate of candidates) {
        try {
            ...
            const citedIds = normalizeSummaryIds(Array.isArray(parsed.citedIds)
                ...
                : undefined);
            const followUpSummaryIds = normalizeSummaryIds(Array.isArray(parsed.followUpSummaryIds)
                ? parsed.followUpSummaryIds.filter((value) => typeof value === "string")
                : undefined);
...
function formatDelegatedExpansionText(passes) {
    const lines = [];
...
export async function resolveRequesterConversationScopeId(params) {
    ...
    try {
        const store = params.lcm.getConversationStore();
