...
import { createDelegatedExpansionGrant, revokeDelegatedExpansionGrantForSession, } from "../expansion-auth.js";
import { remainingDeadlineMs } from "./lcm-expansion-deadline.js";
import { normalizeSummaryIds } from "./lcm-expand-tool.delegation.js";
import { clearDelegatedExpansionContext, recordExpansionDelegationTelemetry, stampDelegatedExpansionContext, } from "./lcm-expansion-recursion-guard.js";
const GATEWAY_TIMEOUT_MS = 10_000;
// Collect nested gateway/provider failure text without exposing object formatting artifacts.
function collectExpansionFailureText(value, parts, depth = 0) {
...
export function formatExpansionFailure(error) {
    const parts = [];
...
function shouldRetryWithoutOverride(message) {
...
function delegatedFailureCode(phase, timedOut) {
    if (timedOut) {
...
function buildDelegatedExpandQueryTask(params) {
    ...
    const messageBackedSummaryIds = params.messageBackedSummaryIds.length > 0
...
function formatInvalidDelegatedReply(reply, reason) {
...
function parseDelegatedExpandQueryReply(rawReply, fallbackExpandedSummaryCount) {
    ...
    for (const candidate of candidates) {
        try {
            ...
            const citedIds = normalizeSummaryIds(Array.isArray(parsed.citedIds)
                ? parsed.citedIds.filter((value) => typeof value === "string")
                : undefined);
            const expandedSummaryCount = typeof parsed.expandedSummaryCount === "number" &&
                Number.isFinite(parsed.expandedSummaryCount)
                ? Math.max(0, Math.floor(parsed.expandedSummaryCount))
                : fallbackExpandedSummaryCount;
            const totalSourceTokens = typeof parsed.totalSourceTokens === "number" && Number.isFinite(parsed.totalSourceTokens)
...
async function runDelegatedQueryAttempt(params) {
    ...
    try {
        ...
        if (remainingDeadlineMs(params.deadline.workDeadlineMs, performance.now()) <= 0) {
            phase = "wait";
            timedOut = true;
            throw new Error(`lcm_expand_query timed out waiting for delegated expansion (${params.delegatedWaitTimeoutSeconds}s).`);
        }
        ...
        createDelegatedExpansionGrant({
            ...
            ttlMs: Math.max(1, remainingDeadlineMs(params.deadline.totalDeadlineMs, performance.now())),
        });
        ...
        const spawnTimeoutMs = Math.max(1, Math.min(GATEWAY_TIMEOUT_MS, remainingDeadlineMs(params.deadline.workDeadlineMs, performance.now())));
        ...
        phase = "wait";
        const waitTimeoutMs = Math.max(1, remainingDeadlineMs(params.deadline.workDeadlineMs, performance.now()));
        ...
        const replyTimeoutMs = remainingDeadlineMs(params.deadline.workDeadlineMs, performance.now());
        if (replyTimeoutMs <= 0) {
            timedOut = true;
            throw new Error(`lcm_expand_query timed out waiting for delegated expansion (${params.delegatedWaitTimeoutSeconds}s).`);
        }
        ...
        const reply = params.deps.readLatestAssistantReply(Array.isArray(replyPayload.messages) ? replyPayload.messages : []);
        if (!reply?.trim()) {
        ...
        const cleanupTimeoutMs = remainingDeadlineMs(params.deadline.totalDeadlineMs, performance.now());
        if (!grantCreated) {
...
export async function runDelegatedExpandQuery(params) {
    ...
    const overrideLabel = overrideProvider && overrideModel
        ? `${overrideProvider}/${overrideModel}`
        : overrideModel || overrideProvider || "configured override";
    params.deps.log.warn(`[lcm] delegated expansion override failed (${overrideLabel}) for conversation ${params.bucket.conversationId}: ${outcome.error}`);
    if (!shouldRetryWithoutOverride(outcome.error)) {
        return outcome;
    }
    params.deps.log.warn(`[lcm] retrying delegated expansion without provider/model override after: ${outcome.error}`);
    return await runDelegatedQueryAttempt({ ...params, task });
}
