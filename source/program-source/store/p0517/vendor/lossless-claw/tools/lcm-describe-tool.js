...
import { getRuntimeExpansionAuthManager, resolveDelegatedExpansionGrantId, } from "../expansion-auth.js";
...
const LcmDescribeSchema = Type.Object({
    ...
    conversationId: Type.Optional(Type.Number({
        description: "Physical conversation ID to scope describe lookups to. If omitted, uses the current session family.",
    })),
    allConversations: Type.Optional(Type.Boolean({
        description: "Set true to explicitly allow lookups across all conversations. Ignored when conversationId is provided.",
    })),
    tokenCap: Type.Optional(Type.Number({
        description: "Optional budget cap used for subtree manifest budget-fit annotations.",
        minimum: 1,
    })),
    expandFile: Type.Optional(Type.Boolean({
        description: "When true (and target is a file_xxx), inline the file's content from disk. " +
    ...
    expandFileMaxBytes: Type.Optional(Type.Number({
        description: "Max bytes of inlined file content when expandFile=true. Default 32768. Hard cap 512000.",
        minimum: 1024,
        maximum: 512_000,
    })),
    startLine: Type.Optional(Type.Number({
        description: "1-based start line for a partial file read when expandFile=true. " +
    ...
    endLine: Type.Optional(Type.Number({
        description: "1-based end line for a partial file read when expandFile=true. Must be >= startLine. " +
            "If omitted with startLine, reads through the last line available within the byte budget.",
        minimum: 1,
    })),
});
function normalizeRequestedTokenCap(value) {
...
export function createLcmDescribeTool(input) {
    return {
        ...
        async execute(_toolCallId, params) {
            ...
            const expandFileMaxBytes = typeof p.expandFileMaxBytes === "number" && Number.isFinite(p.expandFileMaxBytes)
                ? p.expandFileMaxBytes
                : undefined;
            const startLine = typeof p.startLine === "number" && Number.isFinite(p.startLine) ? p.startLine : undefined;
            const endLine = typeof p.endLine === "number" && Number.isFinite(p.endLine) ? p.endLine : undefined;
            ...
            if (conversationScope.conversationId != null
                || (conversationScope.conversationIds?.length ?? 0) > 0) {
                const itemConversationId = result.type === "summary" ? result.summary?.conversationId : result.file?.conversationId;
                const allowedConversationIds = new Set((conversationScope.conversationIds?.length ?? 0) > 0
            ...
            if (result.type === "summary" && result.summary) {
                ...
                const requestedTokenCap = normalizeRequestedTokenCap(params.tokenCap);
                const sessionKey = (typeof input.sessionKey === "string" ? input.sessionKey : input.sessionId)?.trim() ?? "";
                const delegatedGrantId = input.deps.isSubagentSessionKey(sessionKey)
                    ? (resolveDelegatedExpansionGrantId(sessionKey) ?? "")
                    : "";
                const delegatedRemainingBudget = delegatedGrantId !== ""
                ...
                lines.push(`meta conv=${s.conversationId} kind=${s.kind} depth=${s.depth} tok=${s.tokenCount} ` +
                ...
                for (const node of manifestNodes) {
                    lines.push(`d${node.depthFromRoot} ${node.summaryId} k=${node.kind} tok=${node.tokenCount} ` +
                ...
                return {
                    ...
                    details: {
                        ...
                        manifest: {
                            ...
                            budgetSource: requestedTokenCap != null
                                ? "request"
            ...
            if (result.type === "file" && result.file) {
                ...
                if (typeof f.content === "string") {
                    ...
                    if (f.contentTruncated) {
                        lines.push("");
                        lines.push(`*Output truncated to ${f.content.length.toLocaleString()} of ${f.byteSize?.toLocaleString() ?? "?"} bytes. ` +
