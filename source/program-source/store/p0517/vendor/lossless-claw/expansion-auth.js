...
export class ExpansionAuthManager {
    grants = new Map();
    consumedTokensByGrantId = new Map();
    /**
     ...
     */
    getRemainingTokenBudget(grantId) {
     ...
     */
    consumeTokenBudget(grantId, consumedTokens) {
        const grant = this.getGrant(grantId);
        if (!grant) {
            return null;
        }
        const safeConsumed = typeof consumedTokens === "number" && Number.isFinite(consumedTokens)
     ...
     */
    validateExpansion(grantId, request) {
        const grant = this.grants.get(grantId);
        // 1. Grant must exist
        if (!grant) {
 ...
 */
export function resolveDelegatedExpansionGrantId(sessionKey) {
 ...
 */
export function revokeDelegatedExpansionGrantForSession(sessionKey, opts) {
    const key = sessionKey.trim();
    if (!key) {
        return false;
    }
 ...
 */
export function removeDelegatedExpansionGrantForSession(sessionKey) {
    const key = sessionKey.trim();
    if (!key) {
        return false;
    }
 ...
 */
export function resetDelegatedExpansionGrantsForTests() {
 ...
 * orchestrator.
 */
export function wrapWithAuth(orchestrator, authManager) {
    return {
        async expand(grantId, request) {
            ...
            const requestedDepth = typeof request.maxDepth === "number" && Number.isFinite(request.maxDepth)
                ? Math.max(1, Math.trunc(request.maxDepth))
                : grantMaxDepth;
            const effectiveDepth = Math.min(requestedDepth, grantMaxDepth);
            const requestedTokenCap = typeof request.tokenCap === "number" && Number.isFinite(request.tokenCap)
