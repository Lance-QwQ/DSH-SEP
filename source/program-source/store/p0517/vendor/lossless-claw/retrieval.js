...
export class RetrievalEngine {
    ...
    async describeFile(id, options) {
        ...
        if (options?.expandFile === true && file.storageUri && options.largeFilesDir) {
            try {
                const maxBytes = Math.max(1024, Math.min(options.expandFileMaxBytes ?? 32_768, 512_000));
                ...
                const safeRootOk = realTarget === safeRoot || realTarget.startsWith(safeRoot + pathSep);
                if (safeRootOk) {
...
function truncateToValidUtf8End(buf, byteLength) {
    ...
    while (safeEnd > 0) {
        ...
        if ((b & 0xc0) === 0xc0) {
            ...
            const expectedContinuations = (b & 0xf8) === 0xf0 ? 3 : // 11110xxx -> 4-byte sequence
...
function readFileLineRange(fd, fileSize, startLine, endLine, maxBytes) {
