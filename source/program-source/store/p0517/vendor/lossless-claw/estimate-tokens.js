...
function estimateCodePointTokens(cp) {
 ...
 */
function isLikelyOpaqueBinaryString(key, value) {
 ...
 */
const serializedEstimateCache = new WeakMap();
/**
 ...
 */
export function estimateSerializedMessageTokens(message) {
    const cacheable = typeof message === "object" && message !== null;
    if (cacheable) {
        const cached = serializedEstimateCache.get(message);
        if (cached !== undefined) {
            return cached;
        }
    }
    ...
    const serializedEstimate = estimateTokens(serialized) + opaqueParts * OPAQUE_BINARY_PART_TOKEN_ESTIMATE;
    const estimate = Math.max(serializedEstimate, estimateToolResultBoundaryTokens(message));
    if (cacheable) {
        serializedEstimateCache.set(message, estimate);
    }
    return estimate;
}
function estimateBoundaryStringTokens(text, charsPerToken) {
    return Math.ceil(estimateTokens(text) * (4 / charsPerToken));
}
function estimateBoundaryJsonTokens(value, charsPerToken) {
    if (value == null) {
        return 0;
    }
    try {
...
function estimateToolResultBlockTokens(block) {
    ...
    if (block.type === "text" && typeof block.text === "string") {
        return (CONTENT_BLOCK_OVERHEAD_TOKENS +
...
function estimateToolResultBoundaryTokens(message) {
    if (!isToolResultMessage(message)) {
        return 0;
    }
    const tokens = MESSAGE_BOUNDARY_OVERHEAD_TOKENS +
...
export function estimateSerializedMessagesTokens(messages) {
    let total = 0;
 ...
 */
export function truncateTextToEstimatedTokens(text, maxTokens) {
    if (maxTokens <= 0 || !text) {
        return "";
    }
    let tokens = 0;
