import { createIndependentLcmFileLogger, } from "./lcm-file-log.js";
/** Silent logger used when a caller does not provide an explicit sink. */
export const NOOP_LCM_LOGGER = {
    info: () => { },
    warn: () => { },
    error: () => { },
    debug: () => { },
...
function teeLogger(base, fileLogger, options) {
    const fileOnly = (level, message, emit) => {
        if (!fileLogger) {
            emit(message);
            return;
        }
    ...
    const hostAndFile = (level, message, emit) => {
        emit(message);
...
function readRuntimeConfigCandidates(api) {
    const candidates = [];
...
function readOpenClawRedactionConfig(api) {
    ...
    const patterns = Array.isArray(loggingConfig?.redactPatterns)
        ? loggingConfig.redactPatterns.filter((pattern) => typeof pattern === "string")
        : undefined;
    return {
...
function shouldWriteIndependentDebug(api) {
...
export function createLcmLogger(api, config) {
    const fileLogger = createIndependentLcmFileLogger(config.independentLogFile, readOpenClawRedactionConfig(api));
...
export function formatSessionLabel(sessionId, sessionKey) {
    return [
        `session=${sessionId}`,
