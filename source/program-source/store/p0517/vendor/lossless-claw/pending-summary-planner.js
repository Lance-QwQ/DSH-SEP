...
function sortItemsByOrdinal(items) {
    return [...items].sort((a, b) => a.ordinal - b.ordinal);
}
function makeLeafNodeId(prefix, startOrdinal, endOrdinal) {
    return `${prefix}-leaf-${startOrdinal}-${endOrdinal}`;
}
function makeCondensedNodeId(prefix, depth, startOrdinal, endOrdinal) {
...
export function resolvePendingFreshTailOrdinal(input) {
    ...
    const freshTailMaxTokens = typeof input.freshTailMaxTokens === "number" && Number.isFinite(input.freshTailMaxTokens)
    ...
    for (let index = messageItems.length - 1; index >= 0; index--) {
        ...
        const exceedsTokenCap = latestUserProtected &&
            protectedCount > 0 &&
 ...
 */
export function planPendingLeafNodes(input) {
    ...
    const flushChunk = () => {
        ...
        nodes.push({
            ...
            sourceMessageIds: chunk
                ...
                .filter((messageId) => typeof messageId === "number"),
            childNodeIds: [],
            childSummaryIds: [],
        });
        resetChunk();
    ...
    for (const item of items) {
        ...
        const covered = nodes.some((node) => item.ordinal >= node.ordinalStart && item.ordinal <= node.ordinalEnd);
 ...
 */
export function planPendingCondensedNodes(input) {
...
function compareNodesForPlanning(first, second) {
...
function planOneCondensedLayer(input) {
    ...
    return result.filter((node) => !input.nodes.some((existing) => existing.depth === node.depth &&
        existing.ordinalStart === node.ordinalStart &&
        existing.ordinalEnd === node.ordinalEnd));
}
function createCondensedParent(nodeIdPrefix, depth, children, tokenCount) {
    ...
    return {
        ...
        childLinks: children.map((child) => typeof child.canonicalSummaryId === "string"
            ? { childSummaryId: child.canonicalSummaryId }
            : { childNodeId: child.nodeId }),
    };
}
/** Select the highest-depth nodes covering the longest contiguous publish prefix. */
export function selectPendingPublishCoverageTarget(input) {
    ...
    while (cursor <= endOrdinal) {
        const candidate = input.nodes
            .filter((node) => node.ordinalStart === cursor &&
 ...
 */
export function selectPendingPublishFrontier(input) {
