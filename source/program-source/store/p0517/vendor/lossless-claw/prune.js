 ...
 */
function computeCutoffDate(days, now) {
 ...
 */
function resolveMaxBatches(maxBatches) {
    if (maxBatches == null) {
        return null;
    }
 ...
 */
function loadPruneCandidates(db, cutoffDate, limit) {
 ...
 */
function stageCandidateConversationIds(db, candidates) {
    ...
    const insertStmt = db.prepare(`INSERT INTO temp.prune_candidate_ids (conversation_id) VALUES (?)`);
 ...
 */
function dropCandidateConversationIds(db) {
    db.exec(`DROP TABLE IF EXISTS temp.prune_candidate_message_ids`);
    db.exec(`DROP TABLE IF EXISTS temp.prune_candidate_summary_ids`);
    db.exec(`DROP TABLE IF EXISTS temp.prune_candidate_ids`);
}
 ...
 */
function deleteCandidates(db, candidates) {
    ...
    try {
       ...
       WHERE summary_id IN (SELECT summary_id FROM temp.prune_candidate_summary_ids)`).run();
        db.prepare(`DELETE FROM summary_messages
       WHERE message_id IN (SELECT message_id FROM temp.prune_candidate_message_ids)`).run();
        db.prepare(`DELETE FROM summary_parents
       WHERE summary_id IN (SELECT summary_id FROM temp.prune_candidate_summary_ids)`).run();
        db.prepare(`DELETE FROM summary_parents
       WHERE parent_summary_id IN (SELECT summary_id FROM temp.prune_candidate_summary_ids)`).run();
        db.prepare(`DELETE FROM context_items
       WHERE message_id IN (SELECT message_id FROM temp.prune_candidate_message_ids)`).run();
        db.prepare(`DELETE FROM context_items
       WHERE summary_id IN (SELECT summary_id FROM temp.prune_candidate_summary_ids)`).run();
        db.prepare(`DELETE FROM context_items
       WHERE conversation_id IN (SELECT conversation_id FROM temp.prune_candidate_ids)`).run();
        if (tableOptions.hasMessagesFts) {
            db.prepare(`DELETE FROM messages_fts
         WHERE rowid IN (SELECT message_id FROM temp.prune_candidate_message_ids)`).run();
        }
 ...
 */
export function pruneConversations(db, options) {
    const days = parseDuration(options.before);
    if (days == null) {
        throw new Error(`Invalid duration "${options.before}". Expected a value like "90d", "30d", "3m", or "1y".`);
    }
