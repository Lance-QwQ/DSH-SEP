...
export const PROMPT_RECALL_SENSITIVE_IDENTIFIER_PATTERN = /(?:^|[^A-Za-z0-9])(?:ACCESS_?KEY|API_?KEY|AUTH|CREDENTIALS?|DEPLOY_?KEY|KEY|PASS(?:WORD)?|PRIVATE_?KEY|SECRET|TOKEN)(?=$|[^A-Za-z0-9])/i;
export const PROMPT_RECALL_SENSITIVE_VALUE_PATTERN = /(?:-----BEGIN [A-Z ]*PRIVATE KEY-----|\bAKIA[0-9A-Z]{16}\b|\b(?:ghp|gho|ghu|ghs|ghr)_[A-Za-z0-9_]{10,}\b|\bgithub_pat_[A-Za-z0-9_]{20,}\b|\bxox[baprs]-[A-Za-z0-9-]{10,}\b|\b(?:sk|rk|pk)-[A-Za-z0-9_-]{10,}\b|\b(?:sk|rk|pk)_[A-Za-z0-9_]{10,}\b)/i;
export function escapeRegexLiteral(value) {
    return value.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
}
export function isPromptRecallSensitiveIdentifier(identifier) {
    return PROMPT_RECALL_SENSITIVE_IDENTIFIER_PATTERN.test(identifier);
}
export function containsPromptRecallSensitiveMaterial(value) {
    return (PROMPT_RECALL_SENSITIVE_IDENTIFIER_PATTERN.test(value) ||
        PROMPT_RECALL_SENSITIVE_VALUE_PATTERN.test(value));
}
export function findPromptRecallIdentifierIndex(content, identifier) {
    const match = new RegExp(`(^|[^A-Za-z0-9_])${escapeRegexLiteral(identifier)}($|[^A-Za-z0-9_])`).exec(content);
    return match ? match.index + (match[1]?.length ?? 0) : -1;
}
export function findPromptRecallLineStart(content, identifierIndex) {
    const searchStart = Math.max(0, identifierIndex - 1);
    const previousLineBreak = Math.max(content.lastIndexOf("\n", searchStart), content.lastIndexOf("\r", searchStart));
    return previousLineBreak >= 0 ? previousLineBreak + 1 : 0;
}
export function findPromptRecallLineEnd(content, identifierIndex) {
...
export function findPromptRecallSentenceStart(line, relativeIdentifierIndex) {
    let sentenceStart = 0;
...
export function findPromptRecallSentenceEnd(line, relativeIdentifierIndex, identifierLength) {
...
export function clipPromptRecallSnippet(snippet, identifier) {
...
export function extractPromptRecallSnippet(content, identifier) {
...
export function isPromptRecallEligibleRole(role) {
    return role === "user" || role === "assistant";
}
export function extractPromptRecallIdentifiers(prompt) {
