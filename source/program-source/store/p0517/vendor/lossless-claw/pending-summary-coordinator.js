...
import { extractMeaningfulMessageText, resolveLeafSummaryMessageContent, stripInjectedContextBlocks, } from "./compaction.js";
import { estimateTokens } from "./estimate-tokens.js";
import { planPendingCondensedNodes, planPendingLeafNodes, resolvePendingFreshTailOrdinal, selectPendingPublishCoverageTarget, selectPendingPublishFrontier, } from "./pending-summary-planner.js";
import { PendingSummaryPublisher } from "./pending-summary-publisher.js";
import { PENDING_EMPTY_SOURCE_COVERAGE_CONTENT, PENDING_EMPTY_SOURCE_COVERAGE_MODEL, PendingSummaryEmptySourceCoverage, PendingSummaryPreparationWorker, } from "./pending-summary-worker.js";
import { MAX_PENDING_NODE_RETRIES, } from "./store/pending-summary-store.js";
import { LcmProviderAuthError, LcmSummarySpendLimitError, } from "./summarize.js";
...
function digestText(prefix, parts) {
...
function shortDigest(prefix, parts) {
...
async function formatMessageForSummary(store, message, stripInjectedContextTags) {
    const resolved = await resolveLeafSummaryMessageContent(store, message);
    const content = extractMeaningfulMessageText(stripInjectedContextBlocks(resolved, stripInjectedContextTags)).trim();
    if (!content) {
        return "";
    }
...
export class PendingCompactionCoordinator {
    ...
    async runOnce(input) {
        ...
        if (publishPolicy === "publish-ready-only") {
            const publishResult = await this.withPublishLock(() => this.publishActiveBatchIfReady(input.conversationId));
        ...
        const worker = new PendingSummaryPreparationWorker({
            ...
            summarize: (sourceText, node) => this.summarize(sourceText, false, {
    ...
    async extendReadyBatchIfRangeGrew(input) {
        if (!this.hasRemainingCompactableWork(input)) {
            return null;
        }
        if (input.snapshot.compactableStartOrdinal == null ||
        ...
        const extensionItems = input.snapshot.items.filter((item) => item.ordinal > input.batch.compactableEndOrdinal &&
        ...
        const suffixRawTokens = extensionItems.reduce((total, item) => item.itemType === "message" ? total + normalizeNonNegativeInteger(item.tokenCount) : total, 0);
        ...
        const coveringCondensedNodes = reusablePendingNodes.filter((node) => node.kind === "condensed");
        const condensedCandidates = [...canonicalNodes, ...reusablePendingNodes, ...leafNodes].filter((node) => !coveringCondensedNodes.some((cover) => cover.depth > node.depth &&
        ...
        const existingNodeKeys = new Set(existingNodes.map((node) => this.plannerNodeIdentityKey({
            kind: node.kind,
            depth: node.depth,
            ordinalStart: node.ordinalStart,
            ordinalEnd: node.ordinalEnd,
        })));
        const pendingNodes = [...leafNodes, ...condensedNodes].filter((node) => !existingNodeKeys.has(this.plannerNodeIdentityKey(node)));
    ...
    async insertPendingNode(batchId, conversationId, node) {
        ...
        if (node.kind === "leaf") {
            await this.pendingSummaryStore.linkNodeToMessages(node.nodeId, node.sourceMessageIds.map((messageId) => ({ messageId })));
            return;
        }
        await this.pendingSummaryStore.linkNodeToChildren(node.nodeId, node.childLinks ?? [
            ...node.childNodeIds.map((childNodeId) => ({ childNodeId })),
            ...node.childSummaryIds.map((childSummaryId) => ({ childSummaryId })),
        ]);
    }
    async publishIfReady(input) {
        ...
        const publishedByNodeId = new Map(pendingFrontier.map((node, index) => [node.nodeId, published.frontierSummaryIds[index]]));
        return {
            status: "published",
            batchId: input.batch.batchId,
            ...(this.hasRemainingCompactableWork(input) ? { remainingCompactableWork: true } : {}),
    ...
    hasRemainingCompactableWork(input) {
        return (input.snapshot.compactableEndOrdinal != null &&
            input.batch.compactableEndOrdinal < input.snapshot.compactableEndOrdinal);
    }
    async selectPublishFrontier(input) {
    ...
    async publishActiveBatchIfReady(conversationId) {
    ...
    async getActiveBatchStaleReason(input) {
        ...
        for (let ordinal = input.batch.compactableStartOrdinal; ordinal <= input.batch.compactableEndOrdinal; ordinal += 1) {
        ...
        const summaryItemsById = new Map(input.snapshot.items
        ...
        for (const node of nodes) {
            if (node.kind === "leaf") {
                const sourceFingerprints = input.snapshot.items
                    .filter((item) => item.ordinal >= node.ordinalStart &&
    ...
    async pendingNodeChildrenStillCurrent(input) {
    ...
    async buildProjectionSnapshot(conversationId) {
        ...
        const compactableStartOrdinal = compactableItems.length > 0
            ? Math.min(...compactableItems.map((item) => item.ordinal))
            : null;
        const compactableEndOrdinal = compactableItems.length > 0
    ...
    buildBatchSourceFingerprint(input) {
        const compactableItems = input.snapshot.items
            .filter((item) => item.ordinal >= input.startOrdinal &&
    ...
    buildCanonicalSummaryPlannerNodes(snapshot) {
    ...
    pendingRecordToPlannerNode(node) {
        return {
            nodeId: node.nodeId,
    ...
    async loadSourceText(node) {
        if (node.kind === "leaf") {
            ...
            for (const link of messageLinks) {
                ...
                if (!message) {
                    throw new Error(`Pending leaf summary ${node.nodeId} source message ${link.messageId} was not found`);
                }
                chunks.push(await formatMessageForSummary(this.conversationStore, message, this.config.stripInjectedContextTags));
            }
        ...
        for (const child of children) {
            if (child.childNodeId) {
                ...
                if (pendingChild.model === PENDING_EMPTY_SOURCE_COVERAGE_MODEL &&
            ...
            if (child.childSummaryId) {
                ...
                if (canonicalChild.model === PENDING_EMPTY_SOURCE_COVERAGE_MODEL &&
