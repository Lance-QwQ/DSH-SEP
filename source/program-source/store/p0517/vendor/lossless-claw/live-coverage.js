...
import { OPENCLAW_INJECTED_CONTEXT_TAG_NAMES, openClawInboundDecoratedBodiesMatch, stripLeadingOpenClawInboundTimestamp, } from "./openclaw-inbound-metadata.js";
...
export function normalizeSummaryOverlapText(value) {
...
export const FALLBACK_RETRY_PROMPT_MARKER = "[Retry after the previous model attempt failed or timed out]";
export const NORMALIZED_INTER_SESSION_MESSAGE_MARKER = normalizeSummaryOverlapText(INTER_SESSION_MESSAGE_MARKER);
export const NORMALIZED_INTERNAL_TASK_COMPLETION_EVENT_MARKER = normalizeSummaryOverlapText(INTERNAL_TASK_COMPLETION_EVENT_MARKER);
export function hasFallbackRetryPromptMarker(content) {
    return content.split(/\r?\n/).some((line) => line.trim() === FALLBACK_RETRY_PROMPT_MARKER);
}
export function hasCompleteInternalContextBlock(content) {
    const beginIndex = content.indexOf(INTERNAL_CONTEXT_BEGIN_MARKER);
    if (beginIndex < 0) {
        return false;
    }
    return (content.indexOf(INTERNAL_CONTEXT_END_MARKER, beginIndex + INTERNAL_CONTEXT_BEGIN_MARKER.length) >= 0);
}
export function isVolatileLiveInputContent(content) {
    ...
    return (trimmed.startsWith(INTERNAL_CONTEXT_BEGIN_MARKER) &&
...
export function stripTrailingAssistantPrefill(messages, options = {}) {
    const trimmed = messages.slice();
    while (trimmed.length > 0 && trimmed[trimmed.length - 1]?.role === "assistant") {
        const tail = trimmed[trimmed.length - 1];
        if (options.preserveSubstantiveAssistantTail === true &&
            tail &&
...
export function isVolatileLiveInputMessage(message) {
...
export function normalizeLiveMessageForAssemblyReconciliation(message) {
    const stored = toStoredMessage(message);
    if (stored.role !== "system" && stored.role !== "tool") {
        return message;
    }
    const runtimeRole = stored.role === "system" ? "user" : "toolResult";
    const parts = "content" in message
        ? buildMessageParts({
...
export function isSummaryWrapperContent(content) {
    return content.includes("<summary ") && content.includes("</summary>");
}
export function materializeVolatileLiveInputEntries(entries) {
    return entries
        .slice()
...
export function resolveExactAssembledLiveSortIndexes(params) {
    ...
    const liveSortIndexes = new Map();
    const usedAssembledIndexes = new Set();
    for (let liveIndex = params.liveMessages.length - 1; liveIndex >= 0; liveIndex--) {
        const liveSignature = liveSignatures[liveIndex];
        for (let assembledIndex = params.assembledMessages.length - 1; assembledIndex >= 0; assembledIndex--) {
...
export function buildVolatileLiveInputMergedOutput(params) {
    ...
    for (const retainedEntry of params.retained) {
        ...
        if (retainedLiveIndex !== undefined) {
            while (appendedCursor < appendedEntries.length &&
                appendedEntries[appendedCursor].liveIndex < retainedLiveIndex) {
                output.push(appendedEntries[appendedCursor].message);
                appendedCursor++;
            }
        }
    ...
    return sanitizeToolUseResultPairing(output, params.log);
}
export function matchVolatileLiveInputsToCoverageSlots(params) {
    const entryIndexesByLiveText = new Map();
    for (let entryIndex = 0; entryIndex < params.volatileLiveInputs.length; entryIndex++) {
        const entry = params.volatileLiveInputs[entryIndex];
    ...
    const slots = [];
    const candidateSlotIndexesByEntryIndex = params.volatileLiveInputs.map(() => []);
    const addCandidateSlots = (entryIndexes, assembledIndex, slotCount) => {
        const slotIndexes = [];
    ...
    for (const [liveText, entryIndexes] of entryIndexesByLiveText.entries()) {
        for (let assembledIndex = 0; assembledIndex < params.assembledMessages.length; assembledIndex++) {
            const assembledMessage = params.assembledMessages[assembledIndex];
            const assembled = toStoredMessage(assembledMessage);
            if (!isSummaryWrapperContent(assembled.content)) {
                const exactEntryIndexes = entryIndexes.filter((entryIndex) => messagesHaveSameLiveCoverageSignature(assembledMessage, params.volatileLiveInputs[entryIndex].message));
                if (exactEntryIndexes.length > 0) {
                    addCandidateSlots(exactEntryIndexes, assembledIndex, 1);
                }
                continue;
            }
            const representativeEntry = params.volatileLiveInputs[entryIndexes[0]];
            ...
            const entryIndexesBySignature = new Map();
            for (const entryIndex of entryIndexes) {
                const entry = params.volatileLiveInputs[entryIndex];
            ...
            for (const signatureEntryIndexes of entryIndexesBySignature.values()) {
                const firstEntry = params.volatileLiveInputs[signatureEntryIndexes[0]];
            ...
            const genericSlotCount = Math.min(Math.max(0, capacity - exactSlotCount), Math.max(0, entryIndexes.length - exactSlotCount));
            if (genericSlotCount > 0) {
                addCandidateSlots(entryIndexes, assembledIndex, genericSlotCount);
            }
        }
    }
    const slotToEntryIndex = new Map();
    const tryAssignEntry = (entryIndex, visitedSlots) => {
    ...
    for (let entryIndex = 0; entryIndex < params.volatileLiveInputs.length; entryIndex++) {
        tryAssignEntry(entryIndex, new Set());
    }
    const entryToAssembledIndex = new Map();
...
export function liveContentIsRecognizedDecoratedBareBody(params) {
    ...
    if (liveWithoutTimestamp !== liveLeadingTrimmed &&
    ...
    return (trailingWithoutTimestamp !== trailingLine &&
...
const INJECTED_CONTEXT_MARKERS = [
    ...OPENCLAW_INJECTED_CONTEXT_TAG_NAMES.map((tag) => `<${tag}>`),
    INTERNAL_CONTEXT_BEGIN_MARKER,
];
function liveContentCarriesRecognizedInjectedContextMarker(liveContent) {
...
function assembledRowIsStructuralBareCurrentTurn(params) {
    ...
    if (params.assembledIsLastUserRow &&
...
function resolveStructuralCurrentTurnLiveIndex(params) {
    ...
    for (let assembledIndex = params.assembledMessages.length - 1; assembledIndex >= 0; assembledIndex--) {
        if (toStoredMessage(params.assembledMessages[assembledIndex]).role === "user") {
            lastAssembledUserIndex = assembledIndex;
            break;
        }
    }
    for (let liveIndex = params.liveMessages.length - 1; liveIndex >= 0; liveIndex--) {
        const stored = toStoredMessage(params.liveMessages[liveIndex]);
        if (stored.role !== "user") {
            continue;
        }
        if (!stored.content.trim()) {
        ...
        for (let assembledIndex = 0; assembledIndex < params.assembledMessages.length; assembledIndex++) {
            const assembledStored = toStoredMessage(params.assembledMessages[assembledIndex]);
            if (assembledStored.role !== "user") {
                continue;
            }
            if (assembledRowIsStructuralBareCurrentTurn({
...
export function collectUncoveredVolatileLiveInputs(params) {
    ...
    const uncovered = [];
    const coveredEntriesByAssembledIndex = new Map();
    ...
    for (let entryIndex = 0; entryIndex < volatileLiveInputs.length; entryIndex++) {
        const entry = volatileLiveInputs[entryIndex];
...
export function appendUncoveredVolatileLiveInputsWithinBudget(params) {
    ...
    const protectedAssembledIndexes = expandProtectedToolPairIndexes({
        assembledMessages: params.assembledMessages,
        protectedAssembledIndexes: params.protectedAssembledIndexes ?? new Set(),
    });
    ...
    while (retained.length > 0 && estimatedTokens > params.tokenBudget) {
        ...
        for (let evictIndex = 0; evictIndex < retained.length; evictIndex++) {
            const entry = retained[evictIndex];
            const evictAssembledIndexes = toolPairIndexesByIndex.get(entry.index) ?? new Set([entry.index]);
            const candidateEvictsExactLiveTurn = Array.from(evictAssembledIndexes).some((index) => exactLiveProtectedIndexes.has(index));
            const candidateEvictsProtectedTurn = Array.from(evictAssembledIndexes).some((index) => protectedAssembledIndexes.has(index));
            if (candidateEvictsExactLiveTurn || candidateEvictsProtectedTurn) {
                continue;
            }
            const restoredCoveredEntries = Array.from(evictAssembledIndexes).flatMap((index) => uncovered.coveredEntriesByAssembledIndex.get(index) ?? []);
            const candidateRetained = retained.filter((retainedEntry) => !evictAssembledIndexes.has(retainedEntry.index));
            const candidateAppendedEntries = restoredCoveredEntries.length > 0
            ...
            const bestFits = bestCandidate !== undefined && bestCandidate.estimatedTokens <= params.tokenBudget;
        ...
        const removedEntries = retained.filter((entry) => bestCandidate.evictAssembledIndexes.has(entry.index));
...
export function resolveForkBoundedLiveSuffix(params) {
    ...
    const anchorSearchEnd = forkSourceMessageCount > 0
