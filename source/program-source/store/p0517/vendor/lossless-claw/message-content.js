...
export function appendTextValue(value, out) {
...
export const TOOL_CALL_RAW_TYPES = new Set([
    "tool_use",
    "toolUse",
    "tool-use",
    "toolCall",
...
export const TOOL_RESULT_RAW_TYPES = new Set([
    "function_call_output",
    "tool_result",
    "toolResult",
    "tool_use_result",
]);
export const TOOL_RAW_TYPES = new Set([
    ...TOOL_CALL_RAW_TYPES,
    ...TOOL_RESULT_RAW_TYPES,
]);
export const REASONING_RAW_TYPES = new Set([
    "thinking",
    "redacted_thinking",
    "reasoning",
]);
export const REPLAY_CRITICAL_RAW_TYPES = new Set([
    ...TOOL_RAW_TYPES,
    ...REASONING_RAW_TYPES,
]);
 ...
 */
export function stripModelIdentityFromMetadataJson(metadata) {
 ...
 */
export function extractModelIdentityMetadata(message) {
...
export function hasSubstantiveAssistantContent(message) {
...
export function extractStructuredText(value, depth = 0) {
    ...
    if (typeof record.type === "string" && TOOL_RAW_TYPES.has(record.type)) {
        if (safeBoolean(record.toolOutputExternalized)) {
            const externalizedText = extractStructuredText(record.output, depth + 1) ??
...
export function extractReasoningText(record) {
    const chunks = [];
...
function hasRawBlockOfType(value, rawTypes) {
    if (!value || typeof value !== "object") {
        return false;
    }
    if (Array.isArray(value)) {
...
export function serializeRawPayloadContent(message, fallbackContent) {
...
export function normalizeUnknownBlock(value) {
...
export function extractTopLevelReasoningContent(role, topLevel) {
    if (role !== "assistant") {
        return null;
    }
...
export function topLevelReasoningMetadata(reasoning, only = false) {
    if (!reasoning) {
        return {};
    }
    return {
...
export function toPartType(type) {
    switch (type) {
        case "text":
            return "text";
        case "thinking":
...
export function extractMessageContent(content) {
    ...
    if (Array.isArray(content) && content.length > 0 && content.every((item) => typeof item === "object" && item !== null && !Array.isArray(item) &&
        typeof item.type === "string" &&
        (TOOL_RAW_TYPES.has(item.type) ||
            REASONING_RAW_TYPES.has(item.type)))) {
        return "";
    }
...
export function toSyntheticMessagePartRecord(part, messageId) {
    return {
...
export function buildMessageParts(params) {
    ...
    const role = typeof message.role === "string" ? message.role : "unknown";
    const topLevel = message;
    const topLevelToolCallId = safeString(topLevel.toolCallId) ??
    ...
    const topLevelToolName = safeString(topLevel.toolName) ??
        safeString(topLevel.tool_name);
    const topLevelIsError = safeBoolean(topLevel.isError) ??
    ...
    const externalizedFileIds = Array.isArray(topLevel.externalizedFileIds)
        ? topLevel.externalizedFileIds.filter((fileId) => typeof fileId === "string")
        : undefined;
    const fileBlocksExternalized = safeBoolean(topLevel.fileBlocksExternalized);
    const originalByteSize = typeof topLevel.originalByteSize === "number"
    ...
    for (let ordinal = 0; ordinal < message.content.length; ordinal++) {
        ...
        const metadataRecord = block.metadata.raw;
        const rawBlockType = safeString(metadataRecord?.rawType) ?? block.type;
        const partType = toPartType(rawBlockType);
        const rawBlock = metadataRecord && rawBlockType !== block.type
            ? {
                ...metadataRecord,
                type: rawBlockType,
            }
            : (metadataRecord ?? message.content[ordinal]);
        const toolCallId = safeString(metadataRecord?.toolCallId) ??
        ...
        parts.push({
            ...
            toolCallId,
            toolName: safeString(metadataRecord?.name) ??
                safeString(metadataRecord?.toolName) ??
                safeString(metadataRecord?.tool_name) ??
                topLevelToolName,
            toolInput: metadataRecord?.input !== undefined
            ...
            toolOutput: metadataRecord?.output !== undefined
            ...
            metadata: toJson({
                ...
                originalByteSize: typeof metadataRecord?.originalByteSize === "number"
...
export function hasPersistableMessageRole(message) {
...
export function filterPersistableMessages(messages) {
...
export const OPENCLAW_RUNTIME_CONTEXT_SENTINEL = "OpenClaw runtime context for the immediately preceding user message. This context is runtime-generated, not user-author.";
/**
...
export function toStoredMessage(message) {
    ...
    const normalizedContent = "content" in message
        ? normalizeMessageContentForStorage({
            message,
            fallbackContent: content,
        })
        : content;
    const tokenCount = "content" in message
    ...
    const topLevelReasoning = extractTopLevelReasoningContent(typeof message.role === "string" ? message.role : "", message);
    return {
