...
function computeDeferredCompactionRetryDelayMs(attempts) {
...
function shouldBackOffDeferredCompactionFailure(failureSummary) {
    if (failureSummary == null) {
        return false;
    }
    return !/\b(provider auth failure|summary provider circuit breaker is open)\b/i.test(failureSummary);
}
function toMaintenanceRecord(row) {
    return {
        ...
        contextThreshold: typeof row.context_threshold === "number" && Number.isFinite(row.context_threshold)
            ? row.context_threshold
            : null,
        contextThresholdSource: row.context_threshold_source === "override" || row.context_threshold_source === "global"
            ? row.context_threshold_source
            : null,
        contextFreshTailCount: typeof row.context_fresh_tail_count === "number" &&
            Number.isFinite(row.context_fresh_tail_count) &&
            row.context_fresh_tail_count > 0
            ? Math.floor(row.context_fresh_tail_count)
            : null,
        contextLeafChunkTokens: typeof row.context_leaf_chunk_tokens === "number" &&
            Number.isFinite(row.context_leaf_chunk_tokens) &&
            row.context_leaf_chunk_tokens > 0
            ? Math.floor(row.context_leaf_chunk_tokens)
            : null,
        retryAttempts: typeof row.retry_attempts === "number" && Number.isFinite(row.retry_attempts)
...
function mergeMaintenanceRecord(conversationId, existing, patch) {
    return {
        conversationId,
        ...
        lastStartedAt: patch.lastStartedAt !== undefined ? patch.lastStartedAt : existing?.lastStartedAt ?? null,
        lastFinishedAt: patch.lastFinishedAt !== undefined ? patch.lastFinishedAt : existing?.lastFinishedAt ?? null,
        lastFailureSummary: patch.lastFailureSummary !== undefined
        ...
        currentTokenCount: patch.currentTokenCount !== undefined ? patch.currentTokenCount : existing?.currentTokenCount ?? null,
        projectedTokenCount: patch.projectedTokenCount !== undefined
            ? patch.projectedTokenCount
            : existing?.projectedTokenCount ?? null,
        rawTokensOutsideTail: patch.rawTokensOutsideTail !== undefined
            ? patch.rawTokensOutsideTail
            : existing?.rawTokensOutsideTail ?? null,
        contextThreshold: patch.contextThreshold !== undefined
            ? patch.contextThreshold
            : existing?.contextThreshold ?? null,
        contextThresholdSource: patch.contextThresholdSource !== undefined
            ? patch.contextThresholdSource
            : existing?.contextThresholdSource ?? null,
        contextFreshTailCount: patch.contextFreshTailCount !== undefined
            ? patch.contextFreshTailCount
            : existing?.contextFreshTailCount ?? null,
        contextLeafChunkTokens: patch.contextLeafChunkTokens !== undefined
            ? patch.contextLeafChunkTokens
            : existing?.contextLeafChunkTokens ?? null,
        retryAttempts: patch.retryAttempts !== undefined
            ? Math.max(0, Math.floor(patch.retryAttempts))
            : existing?.retryAttempts ?? 0,
        nextAttemptAfter: patch.nextAttemptAfter !== undefined
            ? patch.nextAttemptAfter
            : existing?.nextAttemptAfter ?? null,
        resolutionReason: patch.resolutionReason !== undefined
        ...
        maintenanceRevision: patch.maintenanceRevision !== undefined
...
export class CompactionMaintenanceStore {
    ...
    async getConversationCompactionMaintenance(conversationId) {
        const row = this.db
            .prepare(`SELECT
           conversation_id,
           pending,
    ...
    async saveConversationCompactionMaintenance(record) {
        this.db
            .prepare(`INSERT INTO conversation_compaction_maintenance (
           conversation_id,
           pending,
           requested_at,
           reason,
         ...
         ON CONFLICT(conversation_id) DO UPDATE SET
           ...
           updated_at = datetime('now')`)
            .run(record.conversationId, record.pending ? 1 : 0, record.requestedAt?.toISOString() ?? null, record.reason ?? null, record.running ? 1 : 0, record.lastStartedAt?.toISOString() ?? null, record.lastFinishedAt?.toISOString() ?? null, record.lastFailureSummary ?? null, record.tokenBudget ?? null, record.currentTokenCount ?? null, record.projectedTokenCount ?? null, record.rawTokensOutsideTail ?? null, record.contextThreshold ?? null, record.contextThresholdSource ?? null, record.contextFreshTailCount ?? null, record.contextLeafChunkTokens ?? null, record.retryAttempts, record.nextAttemptAfter?.toISOString() ?? null, record.resolutionReason, record.resolvedAt?.toISOString() ?? null, record.maintenanceRevision);
    }
    ...
    async requestProactiveCompactionDebt(input) {
        ...
        await this.saveConversationCompactionMaintenance(mergeMaintenanceRecord(input.conversationId, existing, {
            pending: true,
    ...
    async closeInactiveCompactionDebt(input) {
        const result = this.db
            .prepare(`UPDATE conversation_compaction_maintenance
         SET pending = 0,
             running = 0,
         ...
         WHERE conversation_id = ?
           ...
           AND EXISTS (
            ...
            .run((input.resolvedAt ?? new Date()).toISOString(), input.conversationId, input.expectedRevision);
    ...
    async markProactiveCompactionFinished(input) {
        ...
        const nextAttemptAfter = input.nextAttemptAfter !== undefined
        ...
        await this.saveConversationCompactionMaintenance(mergeMaintenanceRecord(input.conversationId, existing, {
            ...
            lastFailureSummary: input.failureSummary === undefined
                ? existing?.lastFailureSummary ?? null
