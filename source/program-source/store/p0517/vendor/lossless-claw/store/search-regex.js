...
const NESTED_QUANTIFIER_PATTERN = new RegExp(`${REGEX_QUANTIFIER_PATTERN}\\)${REGEX_QUANTIFIER_PATTERN}`);
const QUANTIFIED_ALTERNATION_GROUP_PATTERN = /\((?:[^()\\]|\\.)*\|(?:[^()\\]|\\.)*\)(?:[+*?]|\{\d)/;
/**
 ...
 */
export function compileSafeSearchRegex(pattern, flags) {
    if (pattern.length > MAX_SEARCH_REGEX_PATTERN_LENGTH ||
