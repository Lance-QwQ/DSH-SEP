...
import { extensionFromNameOrMime, formatFileReference, formatRawPayloadReference, formatToolOutputReference, generateExplorationSummary, parseFileBlocks, } from "./large-files.js";
import { extractStructuredText, hasReplayCriticalRawBlock, RAW_PAYLOAD_EXTERNALIZATION_REASON, serializeRawPayloadContent, } from "./message-content.js";
...
export class LargeFileInterceptor {
    config;
    summaryStore;
    resolveLargeFileTextSummarizer;
    constructor(config, summaryStore, resolveLargeFileTextSummarizer) {
        this.config = config;
        this.summaryStore = summaryStore;
        this.resolveLargeFileTextSummarizer = resolveLargeFileTextSummarizer;
    }
    static BASE64_IMAGE_MAGIC = [
    ...
    static detectBase64ImageType(base64Data) {
    ...
    static extensionForImageMimeType(mimeType) {
    ...
    static normalizeNativeImageBlock(value) {
    ...
    static basenameForImageReference(pathLike) {
    ...
    static inferNativeImageFileName(params) {
        ...
        const rolePrefix = params.role === "assistant"
            ? "assistant"
            : params.role === "system"
                ? "system"
    ...
    static isExternalizedImageReference(value) {
        if (typeof value !== "string")
            return false;
        return LargeFileInterceptor.IMAGE_REFERENCE_REGEX.test(value.trim());
    }
    static isExternalizedReferenceContent(value) {
        const trimmed = value.trim();
        return (trimmed.startsWith("[LCM File:") ||
    ...
    static IMAGE_REFERENCE_REGEX = /^\[(?:(?:User|System|Tool|Assistant) image|Image): [^\]]*LCM file: file_[a-f0-9]{16}\]$/;
    static IMAGE_REFERENCE_REGEX_GLOBAL = /\[(?:(?:User|System|Tool|Assistant) image|Image): [^\]]*LCM file: file_[a-f0-9]{16}\]/;
     ...
     *  size threshold. */
    static isWhollyExternalizedReferenceContent(value) {
        const trimmed = value.trim();
        if (trimmed.length === 0)
            return false;
        if (trimmed.startsWith("[LCM File:") ||
            trimmed.startsWith("[LCM Tool Output:") ||
    ...
    largeFilesDirForConversation(conversationId) {
        return join(this.config.largeFilesDir, String(conversationId));
    }
    async storeImageFileContent(params) {
    ...
    async interceptUserMediaBase64(params) {
    ...
    async interceptPureBase64Image(params) {
     ...
     */
    async rewriteToolInlineImageValue(params) {
        ...
        const nestedKeys = ["output", "content", "result"];
        const rewrittenRecord = { ...record };
        const fileIds = [];
        let changed = false;
    ...
    async interceptInlineImagesInToolMessage(params) {
        if ((params.message.role !== "toolResult" && params.message.role !== "tool") ||
    ...
    async storeLargeFileContent(params) {
        const dir = this.largeFilesDirForConversation(params.conversationId);
        await mkdir(dir, { recursive: true });
    ...
    async externalizeLargeTextPayload(params) {
        if (params.fileId) {
            ...
            if (existing) {
                ...
                const summary = existing.explorationSummary ??
    ...
    async interceptLargeFiles(params) {
        ...
        for (const block of blocks) {
            ...
            const externalized = await this.externalizeLargeTextPayload({
                ...
                formatReference: ({ fileId, byteSize, summary }) => formatFileReference({
                    fileId,
                    fileName: block.fileName,
                    mimeType: block.mimeType,
                    byteSize,
    ...
    async interceptLargeToolResults(params) {
        ...
        for (const item of params.message.content) {
            ...
            const isStructuredToolResult = rawType !== "tool_result" &&
                rawType !== "toolResult" &&
                rawType !== "function_call_output";
            const isPlainTextToolResult = rawType === "text" &&
            ...
            const toolName = safeString(record.name) ??
                topLevelToolName ??
                "tool-result";
            const callId = safeString(record.tool_use_id) ??
            ...
            const externalized = await this.externalizeLargeTextPayload({
                ...
                mimeType: "text/plain",
                formatReference: ({ fileId, byteSize, summary }) => formatToolOutputReference({
                    fileId,
                    toolName: externalizedPayload.toolName,
                    byteSize,
                    summary,
            ...
            const normalizedRawType = rawType === "function_call_output" ? "function_call_output" : "tool_result";
    ...
    async interceptLargeRawPayload(params) {
        ...
        const externalizedFlag = params.message.rawPayloadExternalized;
