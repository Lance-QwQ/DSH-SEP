...
export function restoreRawUserReplay(assembled, live, entries) {
    const carrierParents = new Map();
    if (entries.length === 0)
        return { messages: assembled, carrierParents };
    ...
    for (const [index, message] of live.entries()) {
        ...
        if (message.role !== "user" || typeof key !== "string" || !key)
            continue;
    ...
    const messages = assembled.flatMap(message => {
        ...
        if (!entry || entry.message.role !== "user" || typeof key !== "string" || duplicateKeys.has(key))
            return [message];
        ...
        if (!source || !messagesHaveSameLiveCoverageSignature(message, entry.message) ||
            !messagesHaveSameLiveCoverageSignature(source, entry.message))
            return [message];
        const restored = { ...message, timestamp: source.timestamp, idempotencyKey: key };
        ...
        for (let next = index + 1; next < live.length; next++) {
            ...
            if (carrier.role !== "custom" || Reflect.get(carrier, "customType") !== "openclaw.runtime-context" ||
                !details || details.runtimeContextCarrier !== true || details.source !== "openclaw-runtime-context")
                break;
            replay.push(carrier);
