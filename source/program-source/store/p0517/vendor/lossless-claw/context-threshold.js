...
function ruleMatches(params) {
    ...
    if (rule.match.model) {
        ...
        const candidates = [runtime.modelRef, runtime.model].filter((candidate) => typeof candidate === "string" && candidate.length > 0);
...
function rulePayloadProduces(rule, persisted) {
    if (rule.contextThreshold !== persisted.contextThreshold) {
        return false;
    }
    if (persisted.freshTailCount !== undefined &&
        rule.freshTailCount !== persisted.freshTailCount) {
        return false;
    }
    if (persisted.leafChunkTokens !== undefined &&
        rule.leafChunkTokens !== persisted.leafChunkTokens) {
        return false;
    }
    return true;
}
function resolvedPayloadMatchesPersisted(resolved, persisted) {
...
function describeRuleMatch(rule, runtime) {
    const parts = [];
    if (rule.match.model) {
...
export function persistedContextThresholdOverride(maintenance) {
    if (typeof maintenance.contextThreshold !== "number" ||
...
export function describeResolvedContextThreshold(resolved) {
    return (`threshold=${resolved.contextThreshold} source=${resolved.source}` +
...
export class ContextThresholdResolver {
    ...
    hasWindowRangeRules;
    constructor(globalThreshold, overrides = []) {
        ...
        this.hasWindowRangeRules = this.rules.some(({ rule }) => rule.match.modelContextWindowMin !== undefined ||
            rule.match.modelContextWindowMax !== undefined);
    }
    ...
    resolve(params) {
        ...
        if (!best) {
            return {
                ...
                reason: this.hasWindowRangeRules && runtime.modelContextWindow === undefined
    ...
    couldAnyRuleProduce(params) {
        return this.rules.some((compiled) => rulePossiblyMatches({ compiled, sessionKey: params.sessionKey, runtime: params.runtime }) &&
