...
export function toRuntimeRoleForTokenEstimate(role) {
...
export function firstRuntimeTokenCount(record, keys) {
    if (!record) {
        return undefined;
    }
 ...
 */
export function sumPromptTokensFromUsageRecord(record) {
    if (!record) {
        return undefined;
    }
    ...
    const rawPromptTokens = normalizeNonNegativeInteger(record["prompt_tokens"] ?? record["promptTokens"] ?? record["input_tokens"] ?? record["inputTokens"]);
    if (rawPromptTokens !== undefined) {
        return rawPromptTokens;
    }
    return undefined;
}
export function extractRuntimePromptTokenCount(runtimeContext) {
    ...
    const usageSum = sumPromptTokensFromUsageRecord(asRecord(ctx["usage"]) ?? asRecord(ctx["lastCallUsage"]) ?? null);
    ...
    const promptCacheUsageSum = sumPromptTokensFromUsageRecord(asRecord(promptCache?.["lastCallUsage"]) ?? null);
 ...
 */
export function estimateAgentMessageTokens(messages) {
