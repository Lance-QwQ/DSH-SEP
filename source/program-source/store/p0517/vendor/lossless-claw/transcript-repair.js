...
function getTerminalStopReason(msg) {
    const stopReason = typeof msg.stopReason === "string"
        ? msg.stopReason
...
function isThinkingLikeBlock(block) {
    return (!!block &&
        typeof block === "object" &&
        ["thinking", "redacted_thinking", "reasoning"].includes(String(block.type ?? "")));
}
...
function isEmptyAfterToolUseDrop(content) {
    if (!Array.isArray(content)) {
        return false;
    }
...
function filterAssistantToolUseBlocks(msg, seenToolUseIds, options = {}) {
    ...
    for (const block of content) {
        if (block && typeof block === "object") {
            const rec = block;
            const id = extractToolCallId(rec);
            const isToolUse = !!id && typeof rec.type === "string" && TOOL_CALL_TYPES.has(rec.type);
            if (isToolUse && id) {
    ...
    return { message: { ...msg, content: kept }, dropped };
}
// -- Repair logic (from session-transcript-repair.ts) --
const MISSING_TOOL_RESULT_TEXT = "[lossless-claw] missing tool result in session history; inserted synthetic error result for transcript repair.";
function isSyntheticMissingToolResult(message) {
    ...
    return message.content.some((block) => !!block &&
        typeof block === "object" &&
        block.type === "text" &&
        block.text === MISSING_TOOL_RESULT_TEXT);
}
/** Prefer a candidate only when no result exists or it replaces a synthetic repair. */
function shouldUseCandidateToolResult(existing, candidate) {
    return (!existing ||
 ...
 */
export function sanitizeToolUseResultPairing(messages, log) {
    const out = [];
    const seenToolResultIds = new Set();
    const toolResultPositions = new Map();
    const seenToolUseIds = new Set();
    const movedToolResultIndexes = new Set();
    ...
    let moved = false;
    let changed = false;
    const recordAssistantToolUseDrops = (dropped) => {
    ...
    for (let i = 0; i < messages.length; i += 1) {
        ...
        const deduped = filterAssistantToolUseBlocks(normalizedAssistant, seenToolUseIds, 
        ...
        for (; j < messages.length; j += 1) {
            ...
            if (nextRole === "assistant") {
                ...
                const preview = filterAssistantToolUseBlocks(normalizedNext, new Set(seenToolUseIds), nextTerminal ? { dropAll: true, record: false } : {});
                    ...
                    if (preview.dropped.length > 0) {
                        ...
                        for (let k = j + 1; k < messages.length; k += 1) {
                            ...
                            if (candidate.role === "assistant") {
                                ...
                                const candidateTerminal = getTerminalStopReason(normalizedCandidate) !== null;
                                const candidatePreview = filterAssistantToolUseBlocks(normalizedCandidate, new Set(lookaheadToolUseIds), candidateTerminal ? { dropAll: true, record: false } : {});
                ...
                remainder.push(preview.message);
                continue;
            }
        ...
        const laterResultsById = new Map();
        for (let k = j + 1; k < messages.length; k += 1) {
            if (movedToolResultIndexes.has(k)) {
                continue;
            }
    ...
    if (droppedDuplicateAssistantToolUseCount > 0 && log) {
        log.warn(`[lossless-claw] sanitizeToolUseResultPairing dropped ${droppedDuplicateAssistantToolUseCount} duplicate assistant tool_use block(s)`);
    }
    if (droppedTerminalAssistantToolUseCount > 0 && log) {
        log.warn(`[lossless-claw] sanitizeToolUseResultPairing stripped ${droppedTerminalAssistantToolUseCount} non-pairable terminal assistant tool_use block(s)`);
    }
