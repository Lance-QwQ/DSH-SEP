 ...
 */
export async function readPdf(path, limit, signal) {
    signal.throwIfAborted();
    const entry = await lstat(path);
    if (!entry.isFile())
        throw new OfficeToPdfError('invalid-output', 'The converter output is not a regular file.');
    ...
    try {
        ...
        if (!info.isFile())
            throw new OfficeToPdfError('invalid-output', 'The converter output is not a regular file.');
        if (info.size > limit)
            throw new OfficeToPdfError('output-too-large', 'The converted PDF exceeds maxOutputBytes.');
