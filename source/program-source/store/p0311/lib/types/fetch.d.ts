...
/** Prepared native presentation added by the explicit module execution path. */
export interface PreparedFetchRendering {
    version: 1;
    maxOutputChars: number;
    text: string;
    truncated: boolean;
}
/** The original provider value plus an optional versioned native presentation. */
export type WebFetchValue = WebFetchResult & {
    rendered?: PreparedFetchRendering;
};
/** Configurable limits for this tool's private module executor. */
export type FetchRenderWorkerConfig = WorkerLimits;
/**
...
/**
 * Convert a plain provider value inside the explicit module worker.
 * @param result - Original provider-owned body and retrieval metadata.
 * @param maxOutputChars - Existing source and complete-output character cap.
 * @returns The same text and truncation state as the historical formatter.
 */
export declare function convertFetchOutput(result: WebFetchResult, maxOutputChars: number): RenderedFetch;
/**
 ...
 */
export declare function formatFetchOutput(result: WebFetchValue, maxOutputChars: number): string;
/**
 ...
 */
export declare function fetchMetaFromValue(value: WebFetchValue, maxOutputChars: number): JsonValue;
/**
 ...
 *   {@link formatFetchOutput}) and on source characters converted in the module.
 * @param workerConfig - private module-executor deployment limits.
 */
export declare function applyWebFetchTool(ctx: Context, timeoutMs: number, maxOutputChars: number, workerConfig?: FetchRenderWorkerConfig): void;
export {};
