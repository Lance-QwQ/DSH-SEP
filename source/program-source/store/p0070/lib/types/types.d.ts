 ...
 */
export declare function FsTargetKey(key: string): FsTargetKey;
/**
 ...
 * @returns the same string, branded; no validation is performed.
 */
export declare function FsVersion(v: string): FsVersion;
/**
...
export interface FsWriteOutcome {
    /** Durable recovery operation ID when the provider supports protected mutations. */
    mutationId?: string;
...
/** Metadata returned by a provider's recoverable single-file operations. */
export interface FsMutationSummary {
    mutationId: string;
...
/** Read-only recovery diagnostics; invalid entries are reported and never silently cleared. */
export interface FsRecoveryReport {
    operations: FsMutationSummary[];
    issues: {
        entry: string;
        problem: string;
    }[];
 ...
 */
export type FsErrorCode = 'FS_NOT_FOUND' | 'FS_NOT_DIRECTORY' | 'FS_NOT_TEXT' | 'FS_NOT_REGULAR_FILE' | 'FS_TOO_LARGE' | 'FS_PERMISSION_DENIED' | 'FS_SANDBOX_DENIED' | 'FS_IO_ERROR' | 'FS_STALE_VERSION' | 'FS_NOT_OBSERVED' | 'FS_AMBIGUOUS_EDIT' | 'FS_EDIT_NOT_FOUND' | 'FS_ABORTED' | 'FS_PROTECTION_UNAVAILABLE' | 'FS_BACKUP_LIMIT' | 'FS_UNSAFE_TARGET' | 'FS_RECOVERY_CONFLICT';
/**
 ...
 * message strings.
 */
export declare class FsError extends HarnessError {
