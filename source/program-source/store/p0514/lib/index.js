...
var poisonedSessions = /* @__PURE__ */ new WeakSet();
var inputBase = { task_id: z.uuid() };
...
var taskEvents = (events) => events.filter((event) => event.type === "task/checkpoint-change");
function taskStep(task, id) {
  return task.steps.find((step) => step.id === id) ?? fail("TASK_SCHEMA", "operation step is absent");
}
function installTaskTool(ctx, config) {
  const caps = config;
  if (caps.allowedTools.length === 0 || caps.allowedTools.length > 64 || new Set(caps.allowedTools).size !== caps.allowedTools.length || caps.allowedTools.some((tool) => !/^[A-Za-z0-9][A-Za-z0-9_.-]{0,63}$/.test(tool) || tool === "task_checkpoint")) {
  ...
  let disposed = false;
  const serial = (session, body) => {
    const next = (queues.get(session) ?? Promise.resolve()).catch(() => {
    }).then(body);
    queues.set(session, next);
    return next;
  };
  function assertLive(current) {
  ...
  async function scope(exec) {
    const agent = ctx.agents.currentInitiator();
    if (agent === void 0 || agent !== exec.agent || ctx.agents.get(agent.id) !== agent || ctx.sessions.get(agent.id) !== agent.session) {
      return fail("TASK_AUTHORITY", "task actions require the current live Agent and its exact Session");
    }
    const cwd = agent.session.header.cwd;
    if (cwd === void 0 || cwd.trim() !== cwd || cwd.length === 0 || !isAbsolute(cwd)) return fail("TASK_SCOPE", "task actions require an explicit current project");
  ...
  async function flush(session) {
    try {
      ...
      poisonedSessions.add(session);
      throw error;
    }
  }
  async function state(current, signal) {
    assertLive(current);
    ...
    if (tasks.size > caps.maxTasks || [...tasks.values()].some((task) => task.steps.length > caps.maxSteps || task.operations.length > caps.maxOperations)) {
    ...
    try {
      const durable = await reader.read(0, void 0, { signal });
  ...
  function boundTask(task, current) {
    if (task === void 0) return fail("TASK_NOT_FOUND", "task is absent from this Session");
  ...
  async function commit(current, change) {
    assertLive(current);
    const candidate = { type: "task/checkpoint-change", seq: SessionSeq(current.session.seq), time: Date.now(), data: change };
    ...
    let reserved = 0;
    const reservedBytes = (data) => Buffer.byteLength(JSON.stringify({ type: "task/checkpoint-change", seq: Number.MAX_SAFE_INTEGER, time: Number.MAX_SAFE_INTEGER, data }));
    for (const retained of tasks.values()) {
      ...
      for (const pending of retained.operations.filter((op) => op.state === "prepared")) {
        ...
        const artifacts = step.artifacts.map((file) => ({ ...file, version: FsVersion("\0".repeat(4096)), bytes: caps.maxArtifactBytes }));
        const complete = reservedBytes({ ...envelope, action: "settle", operation: { ...pending, state: "completed", resultCode: "OK", artifacts } });
        const uncertain = reservedBytes({ ...envelope, action: "settle", operation: { ...pending, state: "uncertain", resultCode: "\0".repeat(256), artifacts: [] } });
        reserved += Math.max(complete, uncertain);
      }
    }
    const retainedBytes = taskEvents([...current.session.snapshotEvents(), candidate]).reduce((total, event) => total + Buffer.byteLength(JSON.stringify(event)), 0);
  ...
  function base(current, id, revision) {
    return {
      schema: 1,
      taskId: TaskId(id),
      revision,
      ...
      project: current.project.targetKey
    };
  }
  async function verify(current, step, signal, prior) {
    const facts = [];
    for (const file of step.artifacts) {
      ...
      try {
        ...
        if (before?.type !== "file" || before.size === void 0 || before.size > caps.maxArtifactBytes) fail("TASK_ARTIFACT", "artifact is absent, not regular, or exceeds the byte limit");
        ...
        const matched = evidence.sha256 === file.sha256 && (prior === void 0 || recorded?.version === evidence.version);
  ...
  async function view(current, task, signal, inspect) {
    ...
    const unknown = pending !== void 0 && !owned.get(current.session)?.has(pending.id);
    const mismatch = evidence.some((step) => step.files.some((file) => !file.matched));
    const status = task.cancelled ? "cancelled" : unknown || mismatch ? "needs_reconciliation" : pending !== void 0 ? "executing" : next === void 0 ? inspect ? "complete" : "complete_unverified" : "ready";
  ...
  async function run(input, exec) {
    ...
    const ready = await serial(current.session, async () => {
      ...
      if (input.action === "create") {
        ...
        if (existing !== void 0) {
          const task3 = boundTask(existing, current);
          if (task3.objective !== input.objective || JSON.stringify(task3.steps) !== JSON.stringify(steps)) return fail("TASK_CONFLICT", "task identity already has a different immutable plan");
          return view(current, task3, exec.signal, false);
        }
        const task2 = await commit(current, { ...base(current, input.task_id, 1), action: "create", objective: input.objective, steps });
        return view(current, task2, exec.signal, false);
      }
      ...
      if (input.action === "execute") {
        ...
        const prior = task.operations.find((op2) => op2.id === input.operation_id);
        if (prior !== void 0) {
        ...
        if (task.operations.some((op2) => op2.state !== "completed")) return fail("TASK_RECONCILIATION", "an unresolved operation requires reconciliation before replanning");
        ...
        const op = { id: TaskOperationId(input.operation_id), stepId: TaskStepId(input.step_id), tool: input.tool, argumentsHash, outerCallId: exec.callId, innerCallId: ToolCallId(randomUUID()), intentSeq: current.session.seq, state: "prepared", resultCode: "", artifacts: [] };
        const next2 = await commit(current, { ...base(current, task.id, task.revision + 1), action: "prepare", operation: op });
        ...
        const owner = { controller: new AbortController(), done: Promise.withResolvers() };
        const entries = owned.get(current.session) ?? /* @__PURE__ */ new Map();
        entries.set(op.id, owner);
        owned.set(current.session, entries);
        active = { task: next2, op, owner, args };
        return void 0;
      }
      if (input.expected_revision !== task.revision) return fail("TASK_CONFLICT", "task revision changed; read the current task");
    ...
    try {
      ...
      const result = await ctx.tools.execute({
        name: running.op.tool,
        arguments: running.args,
        callId: running.op.innerCallId,
      ...
      return await serial(current.session, async () => {
        ...
        const op = {
          ...running.op,
          state: completed ? "completed" : "uncertain",
          resultCode: completed ? "OK" : failureCode,
...
// src/index.ts
var name = "task-checkpoint";
var inject = ["agents", "sessions", "sessionPersistence", "tools", "fs"];
function apply(ctx, config) {
  installTaskTool(ctx, Config(config));
}
var Config = z2.object({
  allowedTools: z2.array(z2.string()).required(),
  maxTasks: z2.number().step(1).min(1).max(32).default(8),
  maxSteps: z2.number().step(1).min(1).max(64).default(16),
  maxOperations: z2.number().step(1).min(1).max(128).default(64),
  maxArtifactBytes: z2.number().step(1).min(1).max(16 * 1024 * 1024).default(1024 * 1024),
  maxStateBytes: z2.number().step(1).min(1024).max(16 * 1024 * 1024).default(256 * 1024),
  maxInputBytes: z2.number().step(1).min(256).max(1024 * 1024).default(16 * 1024),
  maxOutputBytes: z2.number().step(1).min(256).max(16 * 1024 * 1024).default(256 * 1024)
});
export {
  Config,
  TaskId,
