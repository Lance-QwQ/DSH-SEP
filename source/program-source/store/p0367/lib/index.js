...
// dsh-host-isolation/candidates/dsh-alpha2-migration-20260917/source/deepseek-harness-ddefc45fbc7f8e46dd73185e68295696d1297887/packages/extensions/tool-cordis/src/index.ts
import { defineTool as defineTool3 } from "@deepseek-ai/dsh-tools";

// dsh-host-isolation/candidates/dsh-alpha2-migration-20260917/source/deepseek-harness-ddefc45fbc7f8e46dd73185e68295696d1297887/packages/extensions/tool-cordis/src/present.ts
...
// dsh-host-isolation/candidates/dsh-alpha2-migration-20260917/source/deepseek-harness-ddefc45fbc7f8e46dd73185e68295696d1297887/packages/extensions/tool-cordis/src/prompt.ts
var CORDIS_SYSTEM_PROMPT = `# Harness runtime inspection
...
// dsh-host-isolation/candidates/dsh-alpha2-migration-20260917/source/deepseek-harness-ddefc45fbc7f8e46dd73185e68295696d1297887/packages/extensions/tool-cordis/src/providers.ts
import { HOST_BUILTIN_INSPECTION } from "@deepseek-ai/dsh-cordis-host-runner";

// dsh-host-isolation/candidates/dsh-alpha2-migration-20260917/source/deepseek-harness-ddefc45fbc7f8e46dd73185e68295696d1297887/packages/extensions/tool-cordis/src/api-catalog.ts
var SERVICE_API = [
  {
  ...
  {
    key: "inspector",
    summary: "Shared Host/Client service fa\xE7ade over the realm's source publisher.",
    description: "Shared Host/Client service fa\xE7ade over the realm's source publisher.",
    methods: [
      {
...
function queryServiceApi(key, services = SERVICE_API) {
  if (key === void 0) {
    return {
      mode: "catalog",
      services: services.map((service2) => ({
        key: service2.key,
        description: service2.summary,
        methods: service2.methods.map((method) => ({ signature: method.signature }))
      }))
    };
  }
...
function queryEventApi(name3, events = EVENT_API) {
  if (name3 === void 0) {
    return {
      mode: "catalog",
  ...
  const event = events.find((candidate) => candidate.name === name3);
  if (event === void 0) throw new Error(`no catalogued Event named "${name3}"`);
  return {
    mode: "event",
    event: {
      name: event.name,
...
var ANY_OUTPUT = { description: "JSON data owned by this inspect provider." };
var SERVICE_INPUT = exactInput("service", "Exact Service key. Omit it for the compact Service and method-signature directory.");
var EVENT_INPUT = exactInput("event", "Exact Event name. Omit it for the compact Event and listener-signature directory.");
var SERVICE_OUTPUT = {
...
var HOST_EVENTS = EVENT_API.filter((event) => !event.name.startsWith("cordis/"));
function hostInspectProviders(ctx) {
  return [
    registration(
      ...
      "listService",
      (input) => queryServiceApi(readExact(input, "service")),
      SERVICE_INPUT,
      SERVICE_OUTPUT
    ),
    registration(
      ...
      "listEvents",
      (input) => queryEventApi(readExact(input, "event"), HOST_EVENTS),
      EVENT_INPUT,
      EVENT_OUTPUT
    ),
...
// dsh-host-isolation/candidates/dsh-alpha2-migration-20260917/source/deepseek-harness-ddefc45fbc7f8e46dd73185e68295696d1297887/packages/extensions/tool-cordis/src/sep-dynamic.ts
var sep_dynamic_exports = {};
__export(sep_dynamic_exports, {
  apply: () => apply,
  inject: () => inject,
  name: () => name
});
import {
  CordisDynamicPackageId as CordisDynamicPackageId2,
  CordisDynamicPluginId as CordisDynamicPluginId2
} from "@deepseek-ai/dsh-cordis-host-runner";
import { createUserMessage } from "@deepseek-ai/dsh-llm";
import { defineTool as defineTool2 } from "@deepseek-ai/dsh-tools";

// dsh-host-isolation/candidates/dsh-alpha2-migration-20260917/source/deepseek-harness-ddefc45fbc7f8e46dd73185e68295696d1297887/packages/extensions/tool-cordis/src/fiber-state.ts
var FiberState = {
  PENDING: 0,
  LOADING: 1,
  ACTIVE: 2,
...
// dsh-host-isolation/candidates/dsh-alpha2-migration-20260917/source/deepseek-harness-ddefc45fbc7f8e46dd73185e68295696d1297887/packages/extensions/tool-cordis/src/sep-dynamic-inspect.ts
...
// dsh-host-isolation/candidates/dsh-alpha2-migration-20260917/source/deepseek-harness-ddefc45fbc7f8e46dd73185e68295696d1297887/packages/extensions/tool-cordis/src/sep-dynamic-present.ts
function presentInspectSelfCall(args) {
  const target = args.pluginId === void 0 ? "dynamic Cordis Plugins" : args.packageId === void 0 ? args.pluginId : `${args.pluginId}/${args.packageId}`;
...
function presentRunCall(args) {
  return {
    card: "generic",
    kind: "execute",
    title: `${args.mode === "update" ? "Update" : "Run"} Cordis Plugin ${args.pluginId} \xB7 ${args.packageId}`
  };
}
...
// dsh-host-isolation/candidates/dsh-alpha2-migration-20260917/source/deepseek-harness-ddefc45fbc7f8e46dd73185e68295696d1297887/packages/extensions/tool-cordis/src/sep-dynamic-prompt.ts
var CORDIS_SYSTEM_PROMPT2 = `# Dynamic Cordis Plugins
...
- Live Plugin and Package definitions exist only in the current process. define itself does not modify repository source, configuration, or disk. When saved-source tools are enabled, explicit save and load preserve source separately; runtime IDs and approvals never survive restart.
...
7. cordis_undefine: stop and delete a live Plugin and its runtime Packages. Saved source and user data remain. Use it only after confirming that the user no longer needs the running definition.
...
// dsh-host-isolation/candidates/dsh-alpha2-migration-20260917/source/deepseek-harness-ddefc45fbc7f8e46dd73185e68295696d1297887/packages/extensions/tool-cordis/src/persistence.ts
import { CordisDynamicPackageId, CordisDynamicPluginId } from "@deepseek-ai/dsh-cordis-host-runner";
import { defineTool } from "@deepseek-ai/dsh-tools";
function owner(exec) {
  if (exec.agent === void 0) throw new Error("Cordis saved-source tools require a current Agent-backed Session");
  exec.signal.throwIfAborted();
  return exec.agent;
}
function registerPersistenceTools(ctx) {
  if (!ctx.dynamicCordisRunner.persistenceEnabled) return;
  ctx.systemPrompt.section({
    name: "tool:cordis-saved-source",
    order: 116,
    text: "Cordis source drafts are enabled for this project. cordis_save stores an exact inspected Package without activating it. cordis_list_saved lists committed versions and individual failures. cordis_load_saved verifies an exact durable identity, version and checksum and binds it to fresh current-session IDs without running it. Use those returned IDs with cordis_run only after an explicit activation decision. Old Plugin, Package, Run and approval IDs cannot survive restart. Client approval is requested anew; awaiting-approval or starting is not success. A bad update does not automatically roll back: inspect the failure, then explicitly run the prior current version. Stopping or undefining removes runtime effects and keeps saved source and user data."
  });
  ctx.tools.register(defineTool({
    name: "cordis_save",
    description: "Save the exact Host/Client source and metadata of one inspected Package as an immutable local draft. This does not run or enable the Plugin. For a later version, provide its original durableId and the latest saved version as expectedVersion. Current Session, project and permission scope must match; in-flight work, conflicting versions and full storage are refused.",
    parameters: {
    ...
    async execute(args, exec) {
      if (args.durableId === void 0 !== (args.expectedVersion === void 0)) throw new Error("durableId and expectedVersion must be supplied together");
      return await ctx.dynamicCordisRunner.savePackage(owner(exec), {
        pluginId: CordisDynamicPluginId(args.pluginId),
        packageId: CordisDynamicPackageId(args.packageId),
        ...args.durableId === void 0 ? {} : { durableId: args.durableId },
  ...
  ctx.tools.register(defineTool({
    ...
    description: "List bounded saved-source version descriptors for this Session and project without returning code or running anything. Unavailable entries remain visible. Select a known version and checksum explicitly; a saved entry is not a health or activation result.",
    parameters: {},
    output: { schema: { type: "json" }, render: (_args, value) => [{ type: "text", text: JSON.stringify(value, null, 2) }] },
    async execute(_args, exec) {
      return { versions: await ctx.dynamicCordisRunner.listSaved(owner(exec)) };
    }
  }));
  ctx.tools.register(defineTool({
    name: "cordis_load_saved",
    description: "Validate and load one exact saved Plugin version into the current Session without activation. Use its fresh returned Plugin and Package IDs with the existing inspect/run/update/stop tools. No old approval is restored, no dependencies are downloaded, and no user data or older snapshot is copied back.",
    parameters: {
...
// dsh-host-isolation/candidates/dsh-alpha2-migration-20260917/source/deepseek-harness-ddefc45fbc7f8e46dd73185e68295696d1297887/packages/extensions/tool-cordis/src/sep-dynamic.ts
var name = "sep-dynamic-cordis-compat";
var inject = ["tools", "systemPrompt", "dynamicCordisRunner", "cordisInspect"];
function requireAgent(exec) {
  if (exec.agent === void 0) throw new Error("Cordis dynamic tools require an Agent-backed session");
  return exec.agent;
}
function apply(ctx) {
  registerPersistenceTools(ctx);
  ctx.systemPrompt.section({
    name: "tool:sep-dynamic-cordis",
    order: ctx.systemPrompt.getSectionOrder("TOOL_CORDIS"),
    text: CORDIS_SYSTEM_PROMPT2
  });
  ctx.tools.register(defineTool2({
    ...
    execute(args, exec) {
      ...
      if (args.packageId !== void 0 && args.pluginId === void 0) {
      ...
      const pluginId = CordisDynamicPluginId2(args.pluginId);
      ...
      return Promise.resolve(inspectSelfPackage(
        ...
        agent,
        pluginId,
        CordisDynamicPackageId2(args.packageId)
      ));
    },
  ...
  ctx.tools.register(defineTool2({
        ...
        oneOf: [
          {
            ...
            properties: {
              ...
              idPrefix: {
                type: "string",
                required: true,
                description: "Suggested semantic prefix of 3\u20136 lowercase English letters; the Host adds an opaque registry-incarnation suffix."
              }
            }
          },
          {
    ...
    execute(args, exec) {
      const plugin = args.plugin.kind === "new" ? { kind: "new", idPrefix: args.plugin.idPrefix } : { kind: "existing", pluginId: CordisDynamicPluginId2(args.plugin.pluginId) };
  ...
  ctx.tools.register(defineTool2({
    ...
    async execute(args, exec) {
      const agent = requireAgent(exec);
      const pluginId = CordisDynamicPluginId2(args.pluginId);
      const packageId = CordisDynamicPackageId2(args.packageId);
  ...
  ctx.tools.register(defineTool2({
    ...
    async execute(args, exec) {
      const receipt = await ctx.dynamicCordisRunner.stop(requireAgent(exec), CordisDynamicPluginId2(args.pluginId));
  ...
  ctx.tools.register(defineTool2({
    ...
    async execute(args, exec) {
      const receipt = await ctx.dynamicCordisRunner.undefine(requireAgent(exec), CordisDynamicPluginId2(args.pluginId));
      if (!receipt.ok) throw new Error(receipt.message);
  ...
  ctx.on("agent/pre-step", async ({ agent, messages, signal }, next) => {
    ...
    const contexts = ids.map((id) => {
      const reference = ctx.dynamicCordisRunner.reference(agent, CordisDynamicPluginId2(id));
...
function referencedPluginIds(messages) {
  ...
  const pattern = /(?:^|\s)@([a-z]{3,6}-(?:[0-9a-f]{32}-)?\d+)(?=\s|$)/g;
...
// dsh-host-isolation/candidates/dsh-alpha2-migration-20260917/source/deepseek-harness-ddefc45fbc7f8e46dd73185e68295696d1297887/packages/extensions/tool-cordis/src/index.ts
var name2 = "tool-cordis";
var inject2 = ["tools", "systemPrompt", "cordisInspect"];
...
async function apply2(ctx) {
  if (ctx.get("dynamicCordisRunner")?.persistenceEnabled) await ctx.plugin(sep_dynamic_exports);
