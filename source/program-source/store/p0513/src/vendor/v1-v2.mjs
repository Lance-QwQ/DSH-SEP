import { assertLocalEvent, localDispositions, remapLocalEvent } from "../extensions.mjs";
import { RELEASED_V0_EVENT_DISPOSITIONS, assertReleasedArtifactRelationships, assertReleasedEventPayload, assertReleasedV1Header, defineReleasedPayloadDisposition, isReleasedAssistantChunkRun, releasedV1SessionFormatCodec } from "./v0-v1.mjs";
...
function transformMessage(state, event, context) {
	...
	recordSepOrigins(state, pending.group, state.mapping.get(event.seq));
...
function finishAttempt(state, context) {
	...
	recordSepOrigins(state, pending.group, state.targetSeq - 1);
	flushBuffered(state, pending, context);
	state.pending = void 0;
}
...
function remapPayloadReferences(event, mapping) {
	if (assertLocalEvent(event)) return remapLocalEvent(event, (seq) => mapOne(seq, mapping, "local task intentSeq"), mapping.get(event.seq)).data;
	const data = record(event.data);
	switch (event.type) {
...
function recordSepOrigins(state, group, targetSeq) {
 for (const span of group.spans) for (let n = 0; n < span.eventCount; n++) state.sepEmbeddedOrigins.set(span.firstSeq + n, targetSeq);
}
