 ...
 */
export async function writePluginEnabled(filename, id, name, enabled) {
    let text;
    try {
    ...
    const target = items.findLast((item, index) => {
        if (!isMap(item) || document.getIn([index, 'id']) !== id || item.has('insert'))
            return false;
        const expectedName = document.getIn([index, 'name']);
        return !expectedName || expectedName === name;
    });
    if (isMap(target)) {
        if (document.getIn([items.indexOf(target), 'disabled']) === !enabled)
            return false;
