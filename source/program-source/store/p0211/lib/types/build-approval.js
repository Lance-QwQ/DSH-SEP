...
async function readPolicy(dir) {
    ...
    if (!isMap(document.contents))
        throw new Error('pnpm-workspace.yaml must be a YAML mapping');
    const builds = document.get('allowBuilds');
    if (builds !== undefined && !isMap(builds))
        throw new Error('allowBuilds must be a YAML mapping');
    ...
    const pending = isMap(builds) ? builds.items.flatMap(({ key, value }) => isScalar(key) && typeof key.value === 'string' && !/[*?]/.test(key.value)
 ...
 */
export async function readPendingBuilds(dir) {
 ...
 */
export async function approveBuilds(dir, names) {
    const { document, pending } = await readPolicy(dir);
    if (names.some(name => !pending.includes(name)))
        throw new ManagementFailure('stale-approval');
    if (names.length === 0)
        return;
...
//# sourceMappingURL=build-approval.js.map