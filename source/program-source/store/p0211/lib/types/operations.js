...
import { DEFAULT_PROFILE_BUNDLES, initProfile, PROFILE_TEMPLATES, readProfileManifest, resolveBundleDir, resolveProfileDir, loadOverlayPatches, } from '@deepseek-ai/dsh-app-boot';
...
async function reconcile(before, dir, anchor, options) {
    ...
    const bundles = previous.filter((name) => {
        if (!beforeDeps.has(name) && !dependencies.includes(name))
            return true;
 ...
 */
export async function runPluginCommand(context, args, options) {
    const dir = context.dir ?? resolveProfileDir(context.profile, context.home);
 ...
 */
export async function viewProfilePackage(dir, spec, options) {
