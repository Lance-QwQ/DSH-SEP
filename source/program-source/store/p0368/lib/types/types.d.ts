...
export type CordisRunStatus = 'awaiting-approval' | 'starting-host' | 'client-pending' | 'running' | 'waiting' | 'rejected' | 'failed' | 'cancelled' | 'stopped';
...
export type DynamicCordisUndefineReceipt = {
    ...
    ok: false;
    reason: 'plugin-missing' | 'transition-in-flight';
    message: string;
};
...
export type DynamicCordisStopResponse = {
    ...
    ok: false;
    reason: 'plugin-missing' | 'not-running' | 'transition-in-flight';
    message: string;
};
...
export type DynamicCordisInvokeResult = {
    ...
    ok: false;
    code: 'plugin-not-running' | 'stale-run' | 'method-not-found' | 'handler-error' | 'transition-in-flight';
} & CordisErrorDetails);
