...
分发修订 `20260921-office-r2`，文档 `20260921-docs-r5`。当前程序图：`a2d3c02193007261560a35a564a786145c4424f0c4a1b28f46e0f6c5a43803a8`。本页区分本轮 Office 组件替换与 r1 的历史许可覆盖；旧报告及旧包原字节保留。

## 本轮当前结论

本包的旧 `@deepseek-ai/libreoffice-kit` Node API 及旧原生平台包已用本项目新写的 `@dsh-sep/office-converter` 和官方 LibreOffice 26.8.0.3 运行库替换。**这不是取得了旧 Kit 缺失源码，也没有把已有 LibreOffice core 源码冒称为旧 Node API 源码。** 旧仓库 404 和无法核验首选源码的记录仍属于历史 r1。

新适配器的 JavaScript、PowerShell、C# 与测试源码随 `DSH-SEP-Source-20260921-office-r2.zip` 交付，以 `SOURCE_FILES.json` 动态映射为准；其原创许可不覆盖官方 LibreOffice。关系 XML 使用已有 saxes 6.0.0；原 ISC 授权与其依赖许可保留。

官方 MSI 的已下载 SHA-256 为 `4aa6c6e1895f4055104effcb556bd3362d20c6ad707c149543304f395ef9db95`，Windows Authenticode 为 Valid，签名者 The Document Foundation。匹配 core／dictionaries／help／translations 四个源码归档均已完整下载、按官方 SHA-256 核验，总量 654,222,716 字节。可访问的固定官方链接、检查时间、下载回执和源码构建材料见运行库 `SOURCE-ACCESS.md`、`ENGINE-PROVENANCE.json` 与 `source-provenance/`，用户入口见 [Office 转换说明](../../OFFICE_CONVERTER.md)。

官方引擎由行政提取取得，没有本地重编译。十个官方 System64 C++ DLL 按原字节复制到 program 供应用本地加载，原目录保留；行政 MSI 数据库与测试生成 bytecode 排除出发行树。布局变换有清单，不能把重包装目录说成原 MSI 逐字节相同。原 `LICENSE.html`、`license.txt`、`NOTICE`、署名与组件许可保留，不添加 SEP 的商业限制。

本轮源码访问与身份核验解决新组件的获取入口；它不证明每个可选外部源码都已下载、全部原生依赖可重建或所有法律义务已被全面认证。sharp/libvips 的既有范围限制继续保留。新图、依赖、许可索引、源码清单、归档哈希和包级测试由本轮重新绑定，不把下述旧 95／97／613 数量直接套到新图。

## r1 历史覆盖记录（保留适用材料与边界）

以下操作、哈希、数量、相对路径和“本次／当前”均描述原 r1 许可同步工作，不是新 Office 实现的来源或验收结果。历史原文逐字节副本在 docs/history/r1/licenses/release/ 下。原有 MIT 与其他有效授权不因替换组件而撤回。
...
## 历史 r1 的 sharp 与 LibreOffice 对应源码调查
...
**历史 r1 的公开发布阻断项：LibreOffice Kit Node API 首选修改源码当时未取得。** p0438 只有打包 JS、声明文件；原声明的 Kit 提交 7f7a6ab…公开查询 404。p0439 已随附的 helper、LibreOffice core 来源、配方和补丁必须保留，但不能替代 Node API 的首选源码。固定 LibreOffice core bce0998…公开可取，不能据此关闭 Node API 缺口。SOURCE-ACCESS.md 中如实区分，未用通用 MPL 文本冒充缺失源码。
