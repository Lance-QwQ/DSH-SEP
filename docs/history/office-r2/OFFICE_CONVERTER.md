...
当前程序图：`a2d3c02193007261560a35a564a786145c4424f0c4a1b28f46e0f6c5a43803a8`。本轮实际转换、异常收尾和安装包验证：**pass：适配器 69/69；真实格式转换三轮 18/18，加 16 项边界共 34/34；超链接保留且零自动请求；宿主服务三轮、原字体字段断言、4 项通信契约及真实桌面预览通过。最终 ZIP 实装和隐私扫描另由包外交付报告绑定。**。本页描述功能契约和限制；结果尚未绑定时不能把契约文字当作全部测试通过。
...
目标为 **Windows 10/11 x64**；官方引擎最低 Windows 版本为 Windows 10，本轮实际执行环境为 Windows 11。未进行本轮 Windows 10 干净虚拟机验证，也不据此宣称 ARM64、Linux 或 macOS 可用。
...
普通链接放行只表示保留可点击链接，不授权转换时主动请求该地址；本轮使用本地回环接收计数器检查正常链接转换是否产生请求，结果须列入上方实际验证摘要，**通过后才可作为本版交付行为**。此检查不等于扫描了全部 XML 的所有语义与全部内嵌对象。旧 CFB 格式有基本文件头检查，但没有因此完成全部二进制内部对象安全审计。
...
适配器源码在 `DSH-SEP-Source-20260921-office-r2.zip`，按 `SOURCE_FILES.json` 逐项对应。LibreOffice 四个完整源归档可单独提供或通过上述链接取得，不重复塞入每份安装运行库。原 `LICENSE.html`、`license.txt`、`NOTICE`、署名和组件许可保留；SEP 自定义许可只管其相应原创部分，不给官方引擎追加商业分发限制。
